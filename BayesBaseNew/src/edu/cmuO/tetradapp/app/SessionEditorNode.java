 ...
 * @version $Revision: 4829 $ $Date: 2005-12-15 18:39:34 -0500 (Thu, 15 Dec
 *          2005) $
    ...
    private void launchEditorVisit() {
        try {
            ...
            EditorWindow editorWindow =
                    new EditorWindow(editor, editor.getName(), "Save", cloned);
...
//        repaint();
    ...
    private static void addListeners(final SessionEditorNode sessionEditorNode,
        ...
        sessionNode.addSessionListener(new SessionAdapter() {
            public void modelCreated(SessionEvent sessionEvent) {
                ...
                // the current editor window is simply closed.  jdramsey
                // 5/18/02
            ...
            public void modelDestroyed(SessionEvent sessionEvent) {
                ...
                // This code is here to allow multiple editor windows to be
                // opened at the same time--if the current model is destroyed,
                // the current editor window is closed. jdramsey 5/18/02
                if (sessionEditorNode.spawnedEditor() != null) {
                    EditorWindow editorWindow = sessionEditorNode.spawnedEditor();
                    editorWindow.closeDialog();
                }
            ...
            public void modelUnclear(SessionEvent sessionEvent) {
                try {
                    ...
                    JOptionPane.showMessageDialog(sessionEditorNode,
                            "Could not make a model for this box.");
                    e.printStackTrace();
                }
            }
        });
    ...
    private JPopupMenu getNotePopup() {
        ...
        help.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                showInfoBoxForModel(sessionNode.getDisplayName(),
                        sessionNode.getModelClasses());
            }
        });
    ...
    private JPopupMenu getPopup() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    if (getSessionNode().getModel() == null) {
                        ...
                        Component centeringComp = SessionEditorNode.this;
                        JOptionPane.showMessageDialog(centeringComp,
                                "Please destroy the current model first.");
                    }
                }
                catch (Exception e1) {
                    Component centeringComp = SessionEditorNode.this;
        ...
        JMenuItem createDescendantModels =
                new JMenuItem("(Re)create models downstream");
        createDescendantModels.setToolTipText("<html>" +
                "Fills in this box and downstream boxes with models," +
                "<br>overwriting any models that already exist.</html>");

        createDescendantModels.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                Component centeringComp = SessionEditorNode.this;

                if (getSessionNode().getModel() != null &&
                        !getSessionNode().getChildren().isEmpty()) {
                    int ret = JOptionPane.showConfirmDialog(centeringComp,
                            "This box is not empty; you will be destroying " +
                                    "all models downstream. Is that OK?",
                            "Confirm",
                            JOptionPane.OK_CANCEL_OPTION,
                            JOptionPane.WARNING_MESSAGE);
        ...
        SessionConfig.getInstance().getModelClassesMap().values();
        ...
        JMenuItem setRepetition =
                new JMenuItem("Set Repeat Factor (for Simulation)...");
        ...
        JMenuItem editSimulationParameters =
                new JMenuItem("Edit Default Parameters...");
        ...
        popup.add(createDescendantModels);
     ...
     */
    private void addEditLogger(JPopupMenu menu){
        SessionNodeWrapper modelNode = (SessionNodeWrapper) getModelNode();
        SessionNode sessionNode = modelNode.getSessionNode();
        Object model = sessionNode.getModel();
        if(model != null){
            final TetradLoggerConfig config = TetradLogger.getInstance().getTetradLoggerConfigForModel(model.getClass());
            if(config != null){
...
//            throw new RuntimeException(missingParentsMessage());
    ...
    private Class getModelClassFromUser(Class[] modelClasses,
        ...
        SessionNode sessionNode = sessionNodeWrapper.getSessionNode();
        ModelChooser chooser = this.config.getModelChooserInstance(sessionNode.getDisplayName());
        Component centeringComp = SessionEditorNode.this;
    ...
    private Class showInfoBoxForModel(String nodeName, Class[] modelClasses) {
        ...
        ModelChooser chooser = this.config.getModelChooserInstance(nodeName);
        Component centeringComp = SessionEditorNode.this;
     ...
     */
    private boolean editParameters(Class modelClass, Params params,
                                   Object[] parentModels)
            throws Exception {
        ...
        SessionNodeModelConfig modelConfig = this.config.getModelConfig(modelClass);
        ParameterEditor paramEditor = modelConfig.getParameterEitorInstance();
        if (paramEditor == null) {
