 ...
 */
public final class SemImEditor extends JPanel {
     ...
     */
    public SemImEditor(SemIm semIm, String graphicalEditorTitle,
        ...
        this.semIm = semIm;
//        semIm.getSemPm().getGraph().setShowErrorTerms(false);
...
//        menuBar.add(graph);
        ...
        menuBar.add(params);
        menuBar.add(new LayoutMenu(semImGraphicalEditor.getWorkbench()));

        add(menuBar, BorderLayout.NORTH);
    }

    private void checkForUnmeasuredLatents(SemIm semIm) {
        ...
        runner.addPropertyChangeListener(new PropertyChangeListener() {
            public void propertyChange(PropertyChangeEvent evt) {
                ...
                if ("graph".equals(evt.getPropertyName())) {
                    ...
                    semImGraphicalEditor.getWorkbench().setGraph(graph);
                }
            }
        });
    }
...
final class SemImGraphicalEditor extends JPanel {
     ...
     */
    public SemImGraphicalEditor(SemIm semIm, SemImEditor editor,
...
//                    resetLabels();
    ...
    private void finishEdit() {
        if (lastEditedObject() != null) {
            ...
            // todo:
            s = ((JLabel) workbench().getLabel(o)).getText();
    ...
    private void resetEdgeLabel(Edge edge) {
        ...
        if (parameter != null) {
            ...
            if (editor.isEditCovariancesAsCorrelations() &&
                ...
                double varA = semIm().getVariance(nodeA);
                double varB = semIm().getVariance(nodeB);
    ...
    private void resetNodeLabel(Node node) {
        ...
        double stdDev = semIm().getStdDev(node);
...
//            Parameter edgeParam = semIm().getSemPm().getEdgeParameter(
...
final class SemImTabularEditor extends JPanel {
    ...
    public SemImTabularEditor(SemIm semIm, SemImEditor editor,
                              int maxFreeParamsForStatistics) {
...
final class ParamTableModel extends AbstractTableModel {
    ...
    public ParamTableModel(SemIm semIm, SemImEditor editor,
                           int maxFreeParamsForStatistics) {
        if (semIm == null) {
    ...
    public int getRowCount() {
        ...
        return semIm().getNumFreeParams() + numNodes;
    }
    ...
    public Object getValueAt(int row, int column) {
        ...
        int numFreeParams = semIm.getNumFreeParams();
        if (row < numFreeParams) {
            Parameter parameter = ((Parameter) freeParameters.get(row));
...
//            DoubleMatrix2D covars = semIm.getImplCovar();
...
//            double var = covars.get(index, index);
        ...
        if (columnIndex == 3) {
            try {
                ...
                if (rowIndex < semIm().getNumFreeParams()) {
                    ...
                    if (parameter.getType() == ParamType.VAR) {
                        value = value * value;
                        semIm.setErrCovar(parameter.getNodeA(), value);
                    } else if (parameter.getType() == ParamType.COEF) {
                        Node x = parameter.getNodeA();
                        Node y = parameter.getNodeB();
...
class ImpliedMatricesPanel extends JPanel {
    ...
    public ImpliedMatricesPanel(SemIm semIm, int matrixSelection) {
        this.semIm = semIm;
...
final class ImpliedCovTable extends AbstractTableModel {
     ...
     */
    public ImpliedCovTable(SemIm semIm, boolean measured,
        ...
        if (measured() && covariances()) {
            ...
            DoubleMatrix2D implCovarC = getSemIm().getImplCovar();
...
final class ModelStatisticsPanel extends JTextArea {
    ...
    private void reset() {
        ...
        if (modelDof >= 0) {
            ...
            append("\nP Value = " + nf.format(modelPValue));
            append("\nBIC Score = " + nf.format(semIm.gelBicScore()));
