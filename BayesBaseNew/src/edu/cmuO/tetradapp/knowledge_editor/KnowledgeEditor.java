 ...
 */
public final class KnowledgeEditor extends JPanel {
     ...
     */
    public KnowledgeEditor(Knowledge knowledge, List<String> varNames,
                           Graph sourceGraph) {
        if (knowledge == null) {
    ...
    private Box getTierBoxes(int numTiers) {
        ...
        List varsNotInTiers = getKnowledge().getVarsNotInTier(getVarNames());
    ...
    private void resetEdgeDisplay() {
        ...
        if (this.showRequiredByGroups) {
            for (Iterator<KnowledgeEdge> i = knowledge.requiredEdgesIterator(); i.hasNext();) {
                ...
                if (knowledge.edgeRequiredByGroups(from, to)) {
        ...
        if (this.showForbiddenByGroups) {
            for (Iterator<KnowledgeEdge> i = knowledge.forbiddenEdgesIterator(); i.hasNext();) {
                ...
                String from = edge.getFrom();
                String to = edge.getTo();
                if (knowledge.edgeForbiddenByGroups(from, to)) {
                    KnowledgeModelNode fromNode = (KnowledgeModelNode) graph.getNode(from);
                    KnowledgeModelNode toNode = (KnowledgeModelNode) graph.getNode(to);
        ...
        testButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    parser.parseKnowledge(text.toCharArray());
        ...
        loadButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
                try {
                    String text = getTextArea().getText();
                    DataParser parser = new DataParser();
                    Knowledge knowledge = parser.parseKnowledge(text.toCharArray());
