...
public final class ColtDataSet
        implements RectangularDataSet, TetradSerializable {
...
//                throw new IllegalArgumentException("Expecting all continuous variables: " + variables);
     ...
     */
    @Override
	public final RectangularDataSet subsetColumns(List<Node> vars) {
//        if (vars.isEmpty()) {
...
//        return !getMultipliers().keySet().isEmpty();
	...
	public final int getMultiplier(int caseNumber) {
        ...
        //System.out.print("!! "+multiplierInt);//zqian
	...
	public final DoubleMatrix2D getCorrelationMatrix() {
        ...
        for (int i = 0; i < data.columns(); i++) {
            for (int j = 0; j < data.columns(); j++) {
                cov.set(i, j, StatUtils.rXY(data.viewColumn(i).toArray(), data.viewColumn(j).toArray()));
            }
        }
...
//        return Statistic.covariance(data);
     ...
     * @see edu.cmu.tetrad.data.DataSavers
     */
    @Override
     ...
     */
    @Override
	public final RectangularDataSet subsetColumns(int indices[]) {
        List<Node> variables = getVariables();
    ...
    @Override
	public final RectangularDataSet subsetRows(int rows[]) {
	...
	public final void shiftColumnDown(int row, int col, int numRowsShifted) {
        ...
        int _lastRow = lastInterestingRow();
	...
	public final boolean equals(Object obj) {
        ...
        if (!(obj instanceof RectangularDataSet)) {
            return false;
        }

        RectangularDataSet _dataSet = (RectangularDataSet) obj;
