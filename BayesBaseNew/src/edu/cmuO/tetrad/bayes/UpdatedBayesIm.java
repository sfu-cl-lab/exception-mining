 ...
 * @version $Revision: 6379 $ $Date: 2006-01-20 13:34:55 -0500 (Fri, 20 Jan
 *          2006) $
 */
public final class UpdatedBayesIm implements BayesIm, TetradSerializable {
     ...
     * retrieved from here.
     *
     * @serial Cannot be null; must be evidence.getSemIm().
     */
    ...
    /**
     * Returns the first BayesIm obtained by recursively calling
     * getDirichletBayesIm() that is not itself an UpdatedBayesIm.
     */
//    private BayesIm getOriginalBayesIm(BayesIm bayesIm) {
//        if (!(bayesIm instanceof UpdatedBayesIm)) {
//            return bayesIm;
//        }
//
//        BayesIm parent = ((UpdatedBayesIm) bayesIm).getBayesIm();
//        return getOriginalBayesIm(parent);
//    }
    @Override
	public Dag getDag() {
     ...
     * probabilities in the BayesIm. It's the outerProduct of the probabilities
    ...
    private boolean[] calcRelevantVars(int nodeIndex) {
        ...
        for (int i = 0; i < bayesIm.getNumNodes(); i++) {
            ...
            if (bayesIm.getDag().isDConnectedTo(node, node2, conditionedNodes))
            {
                relevantVars[i] = true;
            }
        }
