...
public final class EmBayesEstimator {
    ...
    private RectangularDataSet mixedData;   //Contains all variables with missing value columns for
    //latents
...
//    private BayesIm inputBayesIm;
     ...
     */
    public EmBayesEstimator(BayesPm bayesPm, RectangularDataSet dataSet) {
...
//        }
        ...
        for (Node observedVar : observedVars) {
            if (!this.dataSet.getVariables().contains(observedVar)) {
                throw new IllegalArgumentException(
                        "Some observed var in the Bayes net " +
    ...
    public EmBayesEstimator(BayesIm inputBayesIm, RectangularDataSet dataSet) {
...
//        System.out.println(observedIm);
        ...
        RectangularDataSet dsMixed = new ColtDataSet(numFullCases, variables);
...
//                            row);
    ...
    public BayesIm maximization(double threshhold) {
        ...
        while (Double.isNaN(distance) || distance > threshhold) {
            ...
            System.out.println("Distance = " + distance + " at iteration " + iteration);
        }
        return newBayesIm;
    }
     ...
     */
    private void estimateIM(BayesPm bayesPm, RectangularDataSet dataSet) {
        if (bayesPm == null) {
            throw new NullPointerException();
        }
...
//            int[] parentsObs = observedIm.getParents(nodeObsIndex);
    ...
    public RectangularDataSet getMixedDataSet() {
        return mixedData;
    }
