...
import edu.cmu.tetrad.util.SeededRandomUtil;
...
import ca.sfu.jbn.parameterLearning.jointProbLearner;
...
public final class DirichletBayesIm implements BayesIm {
     ...
     */
    public DirichletBayesIm(BayesPm bayesPm) throws IllegalArgumentException {
     ...
     * probability stored in 'pseudocounts', then the maximum number of rows in
     * the table for N is #vals(P1) x #vals(P2) x ... x #vals(Pn).
     *
     * @param nodeIndex
     * @return this number.
     * @see #getRowIndex
	...
	public double getProbability(int nodeIndex, int rowIndex, int colIndex) {
        ...
        if(jointProbLearner.use){
        	int num = bayesPm.getNumCategories(nodes[nodeIndex]);
        	parameterPseudocount = jointProbLearner.getConditionalProbability(nodeIndex, rowIndex, colIndex,pseudocounts, num);
        }
        else
        	parameterPseudocount = getPseudocount(nodeIndex, rowIndex, colIndex);
      //  double rowPseudocount = getRowPseudocount(nodeIndex, rowIndex);

        if (parameterPseudocount < 0.0) {
            return Double.NaN;
        } else {
        	return parameterPseudocount;
//            return parameterPseudocount / rowPseudocount;
     ...
     * be set before calling a randomize method. The default value is 100.
     */
    public void setNextRowTotal(double nextRowTotal) {
     ...
     * it's not.
     *
     * @throws UnsupportedOperationException If you ever try to invoke it.
     */
    @Override
	public RectangularDataSet simulateData(RectangularDataSet dataSet, boolean latentDataSaved) {
        throw new UnsupportedOperationException();
    }
     ...
     */
    public RectangularDataSet simulateData(int sampleSize, long seed,
                                           boolean latentDataSaved) {
        return simulateDataHelper(sampleSize, new SeededRandomUtil(seed),
                latentDataSaved);
    }
     ...
     */
    private RectangularDataSet simulateDataHelper(RectangularDataSet dataSet,
                                                  RandomUtil randomUtil,
                                                  boolean latentDataSaved) {
    ...
    private void constructSample(int sampleSize, RandomUtil randomUtil,
                                 int numMeasured, RectangularDataSet dataSet,
                                 int[] map) {
	...
	public String toString() {
        ...
        for (int i = 0; i < getNumNodes(); i++) {
            ...
            for (int j = 0; j < getNumRows(i); j++) {
                ...
                for (int k = 0; k < getNumColumns(i); k++) {
                    buf.append(nf.format(getProbability(i, j, k)));
                    buf.append("\t");
                }
            }
        }
