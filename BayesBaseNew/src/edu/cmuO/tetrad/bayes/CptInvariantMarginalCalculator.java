...
public final class CptInvariantMarginalCalculator
    ...
    private double getProbabilityOfRow(int variable, final int row) {
        ...
        for (int index = 0; index < parents.length; index++) {
            ...
            if (noModifiedCpts(parents, index)) {
                ...
                Evidence evidence = addConditions(variable, row, index);
    ...
    /**
     * Adds the parent value information to the evidence for parent indices 0
     * to index - 1.
     */
    private Evidence addConditions(int variable, int row, int index) {
        int[] parents = bayesIm.getParents(variable);
        int[] parentValues = bayesIm.getParentValues(variable, row);
        ...
        for (int j = 0; j < index; j++) {
            evidence.getProposition().setCategory(parents[j], parentValues[j]);
        }
    ...
    /**
     * Returns true iff conditioning no parents 0 through i - 1 would change any
