...
package edu.cmu.tetrad.gene.algorithm.biolingua;

import edu.cmu.tetrad.gene.algorithm.util.SymMatrixF;
...
public class Biolingua {
    ...
    private static float bitsErrors = 3;
    private static float bitsLinks = 4;
    private static float bitsPredic = 3;
    ...
    private static BiolinguaDigraph doBiolinguaAlgorithm(
        ...
        */
        ...
        while (true) {
            // Go through all 1 step changes on current model,
            ...
            // changes on current model, we find no improvement in the
            // evaluation metric
            //
            ...
            // one-step change in the current Model that improved
    ...
    /**
     * For each pair of variables (nodes) in the model check all paths that
     ...
     * predictions of all paths between two nodes agree (sign-wise) then choose
     * that sign as the predicted correlation.<p> </p> When the signs predicted
     * by two or more paths disagree, then find out which sign is "dominant"
     * (checking whether there are more + than - paths, or the other way
     ...
     * the # of errors (erroneous predictions) </p> After checking all paths,
    ...
    private static float evalCurrentModel() {
        ...
        for (int vi = 0; vi < nvars; vi++) {
            for (int vj = 0; vj < nvars; vj++) {
                ...
                // Find all paths from vi to vj, counting the number of paths
                ...
                if (pos + neg > 0) {
                    ...
                    // If there are positive as well as negative paths,
                    // increment the # of annotations by 1
                    if ((pos > 0) && (neg > 0)) {
                        annotations++;
                    }

                    // Determine the predicted sign according to all paths
    ...
    // paths, but updates all these counters used  in the eval. metric.
