 ...
 * here to avoid ambiguity), only lags behind the current time step are recorded
 * temporally, with causal edges extending from lagged factors with lags >= 1 to
 * factors in the current time step (lag = 0) only. This "update graph" is
...
public final class BasicLagGraph implements LagGraph {
     ...
     * than this. This value must be >= the current value of getMaxLag().
     */
    @Override
	...
	public Map<String, Point> getLocations() {
        ...
        if (this.locations == null) {
            this.locations = new HashMap<String, Point>();
        }
        return locations;
    }
