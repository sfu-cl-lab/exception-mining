...
/**
 * Checks the independence of latent variables in a measurement model by
 * constructing estimating its parameters under the special conditions
 * illustrated by Figure 12.18 of CPS, 2nd edition. This is basically a test of
 * the rank of the covariance matrix of latents.
 *
...
public final class IndTestMimBuild implements IndependenceTest {
    ...
    /**
     * Constructs a new IndependenceChecker which checks independence facts
    ...
    public IndTestMimBuild(RectangularDataSet dataSet, double sig,
    ...
    public IndTestMimBuild(CovarianceMatrix covMatrix, double sig,
            Clusters measurements) {
    ...
    public void setData(RectangularDataSet dataSet) {
    ...
    public void setMeasurementsSource(Clusters clusters) {
        ...
        for (String varName : clusters.getClusters().keySet()) {
            ...
            for (Object aListRelated : listRelated) {
                ...
                measurements.setEdgeRequired(latent_string, varName, true);
            }
        }
        initMeasurements();
    }
...
//        }
        ...
        CovarianceMatrix newCov = covMatrix.getSubmatrix(variables);
...
//    }
    ...
    private double[][][] getBootstrapSamples(int numSamples) {
        ...
        RectangularDataSet dataContinuous = getData();
...
//        double dummyData[][] = new double[numColumns][sampleSize];
        RectangularDataSet dummyDataSet =
...
//        }
        ...
        for (int iter = 0; iter < numSamples; iter++) {
            for (int i = 0; i < sampleSize; i++) {
                int row =
                        PersistentRandomUtil.getInstance().nextInt(sampleSize);
...
//                    dummyData[j][i] = rawData[row];
    ...
    /**
     * Change the order of the elements in the latents list so they correspond
     * to the order of the elements in this semPm. The reason is that we want
     * the position of elements in the bootstrapped latent covariance matrices
     * to correspond to the position of the latents in the latents list
     */
    ...
    private void fixLatentOrder(SemPm semPm) {
        ...
        latents = newLatents;
        System.out.println(latents.toString());
    }
