...
*                             determining which regressors are taken to be
    ...
    public RegressionResult(boolean zeroInterceptAssumed, String[] regressorNames,
                            ...
                            double rss, double alpha, DoubleMatrix1D yHat, DoubleMatrix1D res) {
        ...
        if (b.length != getNumRegressors() + error) {
...
//        }
    ...
    public double getPredictedValue(double[] x) {
        ...
        for (int i = offset; i < getNumRegressors(); i++) {
            yHat += b[i + offset] * x[i];
        }
	...
	public String toString() {
        StringBuffer summary = new StringBuffer(getPreamble());
