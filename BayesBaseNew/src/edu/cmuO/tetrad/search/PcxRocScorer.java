 ...
 * different. </p> The member variables scores and inCategory are used as input
 * to an instance of an ROC procedure which calculated the ROC curve and the AUC
 * or Area Under the Curve.
 *
 * @author Frank Wimberly based on a specification by Clark Glymour
 * @version $Revision: 5758 $ $Date: 2006-01-20 15:58:02 -0500 (Fri, 20 Jan
 *          2006) $
 */
public class PcxRocScorer implements DiscreteClassifier {

    /**
     * The dataset used to create the instantiated Bayes net.
     */
    private RectangularDataSet ddsTrain;

    /**
     * The dataset to be classified.
     */
    private RectangularDataSet ddsClassify;

//    /*
//     * The raw data matrix for the dataset to be classified restricted to the variables
...
//    private int[][] rawData;
    ...
    /**
     * The target value of the target variable.  This is the value which
     * defines the property P that each case either has or does not have.
     */
    ...
    /**
     * The double array scores contains the score for each case which is used in
     * the ROC calculation.
     */
    private double[] scores;

    /**
     * The boolean array inCategory indicates, for each case, it has category P.
     * It has category P if the value of the target variable in that case is
     * the target getValue.
     */
    private boolean[] inCategory;

    /**
     * The int numValidCases indicates the number of cases for which valid
     * marginals were computed.  This will be the number of meaningful entries
     * in the arrays scores and inCategory.
     */
     ...
     */
    public PcxRocScorer(RectangularDataSet ddsTrain,
            RectangularDataSet ddsClassify, String target, int targetValue,
            double alpha, int depth) {

        this.ddsTrain = ddsTrain;
        this.ddsClassify = ddsClassify;
        this.target = target;
	...
	public int[] classify() {
        ...
        IndTestChiSquare test = new IndTestChiSquare(ddsTrain, alpha);
        //        IndTestGSquare2 test = new IndTestGSquare2(ddsTrain, alpha);
        ...
        PcxSearch pcxs = new PcxSearch(test, depth);
        Graph markovBlanket = pcxs.search(target);
        ...
        this.markovBlanketNodes = markovBlanket.getNodes();
        ...
        RectangularDataSet ddsMBTrain =
                ddsTrain.subsetColumns(markovBlanketNodes);

        //The variables that correspond to the nodes in the Markov blanket.
        List varsTrain = ddsMBTrain.getVariables();
        ...
        RectangularDataSet ddsMBClassify =
                ddsClassify.subsetColumns(markovBlanketNodes);

        //Get the raw data from the dataset to be classified, the number
        //of variables and the number of cases.
//        this.rawData = ddsMBClassify.getIntMatrixTransposedTrimmed();
        int nvars = ddsMBClassify.getNumColumns();
        int ncases = ddsMBClassify.getNumRows();

        this.scores = new double[ncases];
        this.inCategory = new boolean[ncases];

        int[] estimatedValues = new int[ncases];
        Arrays.fill(estimatedValues, -1);

        //The variables in the dataset.
        List varsClassify = ddsMBClassify.getVariables();
        ...
        int numValid = 0;    //The number of valid cases (with valid marginals).
        for (int i = 0; i < ncases; i++) {
            ...
            for (int j = 0; j < varsClassify.size(); j++) {
                ...
                String other =
                        ((DiscreteVariable) varsClassify.get(j)).getName();
            ...
            //Set arrays used in creating the ROC calculator.
            int j = varsClassify.indexOf(targetVariable);
            boolean hasPropertyP = false;

            double targetValueProb =
                    bayesUpdaterMb.getMarginal(indexTargetBN, targetValue);
            if (ddsMBClassify.getInt(i, j) == targetValue) {
                hasPropertyP = true;
            }
            //System.out.println(i + " " + targetValueProb + " " + hasPropertyP);
            ...
            if (estimatedValue < 0) {
                ...
                for (int m = 0; m < nvars; m++) {
                    System.out.print(
                            ((DiscreteVariable) varsClassify.get(m)).getName());
                    TetradLogger.getInstance().details("  " + ddsMBClassify.getInt(i, m));
                }
                continue;
            }

            //The score for the case and the boolean that indicates whether it has
            //property P (the value of the target variable is the target value.
            scores[numValid] = targetValueProb;
            inCategory[numValid] = hasPropertyP;
            numValid++;
    ...
    /**
     * @return the array of scores for cases used in the ROC calculation.
     */
    ...
    /**
     * @return the array of booleans indicating for each case whether the
     *         value of the target variable is the target value (i.e.
     *         whether it has property P.
     */
    ...
    /**
     * @return the number of cases for which results are stored in scores and
     *         inCategory.
     */
    public int getNumValidCases() {
        return numValidCases;
    }
}
