...
public class TestMarkovBlanketSearches extends TestCase {
    ...
    public static void testSubgraph1() {
        ...
        MarkovBlanketSearch search = new GrowShrink(test);
    ...
    public static void testSubgraph2() {
        ...
        IndTestDSep test = new IndTestDSep(graph);
        MarkovBlanketSearch mbSearch = new GrowShrink(test);
...
//        LogUtils.getInstance().add(System.out, Level.FINEST);
        ...
        List<MarkovBlanketSearch> algorithms =
                new LinkedList<MarkovBlanketSearch>();
        ...
        for (int n = 0; n < params.getNumTests(); n++) {
            ...
            if (params.isRandomGraphEveryTime() || createRandomGraph) {
                randomGraph = GraphUtils.createRandomDag(params.getNumVars(), 0, numEdges,
                        9, 3, 9, false);
            ...
            while (tried <= 30) {
                i = PersistentRandomUtil.getInstance().nextInt(params.getNumVars());
...
//                if (trueMbDag.getNumNodes() != minMbSize + 1) {
            ...
            for (MarkovBlanketSearch algorithm : new LinkedList<MarkovBlanketSearch>(algorithms)) {
...
//                printMbfsGraphStats(t, test, depth, trueMbDag, i, nf2);
        ...
        for (MarkovBlanketSearch algorithm : algorithms) {
...
//                    algorithm.getAlgorithmName());
    ...
    private MarkovBlanketSearch getAlgorithm(String name, IndependenceTest test,
            int depth, RectangularDataSet dataSet) {
        if ("PCMB".equals(name)) {
            return new PcMarkovBlanketSearch(test, depth);
        }
        else if ("CPCMB".equals(name)) {
            return new CpcMarkovBlanketSearch(test, depth);
        }
            ...
            return new VanderbiltMmmb(test, depth, false);
        }
        else if ("MMMB-SYM".equals(name)) {
            return new VanderbiltMmmb(test, depth, true);
        }
        else if ("MBFS".equals(name)) {
            return new MbFanSearch(test, depth);
        }
    ...
    private Stats printNodeStats(MarkovBlanketSearch algorithm, Node t,
            List<String> _truth, int i, PrintWriter out, long timeLimit) {
        final long time = System.currentTimeMillis();

        class MyThread extends Thread {
            private MarkovBlanketSearch algorithm;
            ...
            public MyThread(MarkovBlanketSearch algorithm, Node t) {
...
//        System.out.println("FP" + mbfFp);
    ...
    private Stats printGraphStats(MarkovBlanketSearch algorithm, Node target,
            Graph trueMbDag,
            RectangularDataSet dataSet, int i,
        ...
        if (algorithm instanceof MbFanSearch) {
            ...
            estimatedMbDag = ((MbFanSearch) algorithm).getGraph();
        }
        else {
            ...
            RectangularDataSet _dataSet = dataSet.subsetColumns(_nodes);

            GesSearch search = new GesSearch(_dataSet);
    ...
    private Stats printMbfsGraphStats(Node target, IndependenceTest test,
        ...
        MbFanSearch algorithm = new MbFanSearch(test, depth);
    ...
    private RectangularDataSet simulateDiscrete(Dag randomGraph,
    ...
    private RectangularDataSet simulateContinuous(Dag randomGraph,
            int sampleSize, RectangularDataSet dataSet) {
//        System.out.println("Simulating continuous...");
        ...
        if (dataSet == null) {
            dataSet = simulator.simulateDataAcyclic(sampleSize);
        }
        else {
            dataSet = simulator.simulateDataAcyclic(dataSet);
        }
...
//        System.out.println("...simulated.");
    ...
    private static class Stats {
        ...
        public Stats(MarkovBlanketSearch algorithm, int fp, int fn, int errors,
                int truth, long time) {
            this.algorithm = algorithm;
            this.fp = fp;
            this.fn = fn;
        ...
        public MarkovBlanketSearch getAlgorithm() {
            return algorithm;
        }
