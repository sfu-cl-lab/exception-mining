...
/**
 * This class contains a method classify which uses an instantiated Bayes net
 * (BayesIm) provided in the constructor.  For each case (record) in  the
 * DataSet it uses the values of all variables but the target variable to update
 * the distributions of all the variables.  It then computes an estimated value
 * for the target variable by selecting the value with the greatest probability
 * in the updated distribution.  The method returns a crosstabulation table in
 * the form of a two-dimensional integer array in which counts of observed
 * versus estimated values of the target variable are stored. </p> Note that the
 * variables must be the same in the dataset and the Bayes net.
 *
...
public final class BayesUpdaterClassifier
     ...
     * @serial
     */
    private RectangularDataSet testData;
	...
	public int[] classify() {
        ...
        RectangularDataSet selectedData = testData.subsetColumns(varIndices);
        ...
        for (int i = 0; i < ncases; i++) {
            ...
            if (estimatedValue < 0) {
                ...
                for (int m = 0; m < nvars; m++) {
                    ...
                    //        .get(m).getName());
                    TetradLogger.getInstance().details("  " + selectedData.getDouble(i, m));
                }
	...
	public int[][] crossTabulation() {
        ...
        // Create a cross-tabulation table to store the counts of observed
    ...
    public RectangularDataSet getTestData() {
        return testData;
    }
