...
/**
 * Performs a Bayesian classification of a test set based on a given
 * training set. MBFS is used to select a Markov blanket DAG of the target;
 * this DAG is used to estimate a Bayes model using the training data. The
 * Bayes model is then updated for each case in the test data to produce
 * classifications.
 *
...
public class MbClassify implements DiscreteClassifier {
    private RectangularDataSet train;
    private RectangularDataSet test;
     ...
     * @param test The test data. Should be discrete and should contain all
     * of the relevant variables from the training data.
     * @param target The name of the target variable. Should be in the
     * trainin data.
     * @param alpha The significance level for MBFS.
     * @param depth The depth for MBFS.
     * @param prior The symmetric alpha for the Dirichlet prior for Dirichlet
     * estimation.
     * @param maxMissing The maximum number of missing values allowed. Cases
     * with more than this number of missing values among the variables in the
     * DAG found by MBFS will be skipped.
     */
    public MbClassify(RectangularDataSet train, RectangularDataSet test,
    ...
    public MbClassify(String trainPath, String testPath, String targetString,
        ...
        try {
            ...
            TetradLogger.getInstance().info(buf.toString());
...
//            parser.setVarNamesSupplied(false);
            parser.setMaxIntegralDiscrete(50);

            RectangularDataSet train = parser.parseTabular(new File(trainPath));

            parser.setKnownVariables(train.getVariables());
            RectangularDataSet test = parser.parseTabular(new File(testPath));
    ...
    private void setup(RectangularDataSet train, RectangularDataSet test, String target, double alpha,
    ...
    /**
     * Classifies the test data by Bayesian updating. The procedure is as
     * follows. First, MBFS is run on the training data to estimate an MB
     * pattern. Bidirected edges are removed; an MB DAG G is selected from the
     * pattern that remains. Second, a Bayes model B is estimated using this G
     * and the training data. Third, for each case in the test data, the marginal
     * for the target variable in B is calculated conditioning on values of the
     * other varialbes in B in the test data; these are reported as
     * classifications. Estimation of B is done using a Dirichlet estimator,
     * with a symmetric prior, with the given alpha value. Updating is done
     * using a row-summing exact updater.
     * <p/>
     * One consequence of using the row-summing exact updater is that
     * classification will be fast except for cases in which there are lots of
     * missing values. The reason for this is that for such cases the number of
     * rows that need to be summed over will be exponential in the number of
     * missing values for that case. Hence the parameter for max num
     * missing values. A good default for this is like 5. Any test case with
     * more than that number of missing values will be skipped.
     *
	...
	public int[] classify() {
        ...
        TetradLogger.getInstance().setTetradLoggerConfig(new DefaultTetradLoggerConfig("info", "details"));
...
//        Mmmb search = new Mmmb(indTest, depth);
        ...
        RectangularDataSet subset = train.subsetColumns(mbPlusTarget);

        System.out.println("subset vars = " + subset.getVariables());

        PcSearch patternSearch = new PcSearch(new IndTestChiSquare(subset, 0.05), new Knowledge());
...
//        Graph mbPattern = search.search(target);
        ...
        TetradLogger.getInstance().details("Pattern = " + mbPattern);
        MbUtils.trimToMbNodes(mbPattern, train.getVariable(target), true);
        TetradLogger.getInstance().details("Trimmed pattern = " + mbPattern);
        ...
        TetradLogger.getInstance().details("Selected DAG = " + selectedDag);
        TetradLogger.getInstance().details("Vars = " + selectedDag.getNodes());
        ...
        RectangularDataSet trainDataSubset = train.subsetColumns(mbNodes);
        ...
        TetradLogger.getInstance().info("Estimating Bayes net; please wait...");
        ...
        RectangularDataSet testSubset = this.test.subsetColumns(mbNodes);
        ...
        for (int k = 0; k < numCases; k++) {
            ...
            if (numMissing > this.maxMissing) {
                TetradLogger.getInstance().details("classification(" + k + ") = " +
            ...
            TetradLogger.getInstance().details("classification(" + k + ") = " + estimatedCategory);
...
//            }
        ...
        double percentCorrect1 =
                100.0 * (numberCorrect) / (numberCounted);

        // Print the cross classification.
        TetradLogger.getInstance().details("");
        TetradLogger.getInstance().details("\t\t\tEstimated\t");
        TetradLogger.getInstance().details("Observed\t");
        ...
        TetradLogger.getInstance().details(buf0.toString());
        ...
        for (int k = 0; k < numCategories; k++) {
            ...
            TetradLogger.getInstance().details(buf.toString());
        }

        TetradLogger.getInstance().details("");
        TetradLogger.getInstance().details("Number correct = " + numberCorrect);
        TetradLogger.getInstance().details("Number counted = " + numberCounted);
        TetradLogger.getInstance().details(
                "Percent correct = " + nf.format(percentCorrect1) + "%");
    ...
    /**
     * Returns the cross-tabulation from the classify method. The classify
    ...
    /**
     * Returns the percent correct from the classify method. The classify method
     * must be run first.
     */
    @Override
