...
/**
 * <p>Checks independence facts for time series data. The method is described in
 * Alessio Moneta, "Graphical Models for Structural Vector
 * Autoregressions."</p>
 *
...
public final class IndTestTimeSeries implements IndependenceTest {
    ...
    /**
     * The number of indices k for which the k-th row of the data is regressed
     * onto rows k - 1, k - 2, ..., k - numLags().
     *
     * @serial
     */
    private int numReps;
    ...
    /**
     * <p>One plus the the number of previous rows in the data that each
     * examined row is regressed onto.</p>
     *
     * @serial
     */
    private int numLags;
    ...
    /**
     * Stored Sigma_u; this only needs to be calculated once but gets reused a
     * lot.
     */
    ...
    /**
     * An indexed version of sigmaU; used repeatedly to calculate basic tau
     * gradients.
     */
    private transient IndexedMatrix indexedCorr;

    /**
     * True if the stationary algorithm is to be used; false if the non-
     * stationary algorithm is to be used.
     *
     * @serial
     */
    ...
    /**
     * Constructs a new independence test based on Moneta, "Graphical Models for
     * Structural Vector Autoregressions."
     *
     * @param data Row major matrix of data for each of the variables in vars
     *             for each time step. Dimensions numTimeSteps x numVars. Time
     *             steps are assumed to be in increasing order, so that
     *             time(data[i][]) < time(data[j][]) for i < j.
     * @param vars The variables over which the data is (repeatedly) measured.
     *             The number of variables must equal the number of columns in
     *             the data-- that is, vars.size() == data[i].length for each
     *             i.
     */
    public IndTestTimeSeries(double[][] data, List<Node> vars) {
        if (data == null) {
        ...
        this.numTimeSteps = data.length;
        this.numVars = data[0].length;
        this.vars = Collections.unmodifiableList(vars);
        this.data = MatrixUtils.copy(data);
        this.setNumLags(1);
        this.setAlpha(0.05);
    }
    ...
    /**
     * Determines whether variable x is independent of variable data given a
     * list of up to four conditioning vars z; beyond four conditioning
     * variables, false is always returned.
     *
    ...
    /**
     * Returns true iff according to the test var(indices[0]) _||_
     * var(indices[1] | var(indices[2], ..., var(indices[indices.length - 1]).
     */
...
//    }
    ...
    /**
     * Sets the significance level for statistical tests. By default, this is
     * 0.05.
     */
    @Override
    ...
    /**
     * Sets the number of lags. Note that the number of lags plus the lag size
     * must be <= the number of times.
     */
    ...
    /**
     * Needed for the IndependenceTest interface.  Probably not meaningful
     * here.
     */
    @Override
    ...
    /**
     * Returns the row of the data indexed so that the last row has index
     * numReps and the first row has index numReps - numTimeSteps + 1.
     *
...
//    }
    ...
    /**
     * Returns the chi square cutoff value for the given degrees of freedom
    ...
    /**
     * Calculates the gradient of tau for the given indices array (for up to 4
     * conditioning vars).
     */
    private double tau() {
    ...
    /**
     * Takes a gradTau for a basic case and transforms it into a gradTau for the
     * case at hand.
     */
    ...
    /**
     * Returns grad(tau) for numTimeSteps = 2, indArr = [1 2] (one-indexed).
     * From Mathematica. </p> Examples of the gradients of the tau functions
     * were calculated using the following Mathematica script (in this case
     * comparing v1 and v4 given v2 and v3). These were then modified using
     * Xemacs into Java formulas. (In case anyone needs to do it again.)</p>
     * <pre>
     * numVars = 6;
     * indexSub = {1, 4, 2, 3};
    ...
    /**
     * Returns grad(tau) for numTimeSteps = 3, indArr = [1 2 3] (one-indexed).
     * From Mathematica.
     */
    ...
    /**
     * Returns grad(tau) for numTimeSteps = 4, indArr = [1 2 3 4] (one-indexed).
     * From Mathematica.
     */
    ...
    /**
     * Returns grad(tau) for numTimeSteps = 5, indArr = [1 2 3 4 5]
     * (one-indexed). From Mathematica.
     */
    ...
    /**
     * Returns grad(tau) for numTimeSteps = 6, indArr = [1 2 3 4 5 6]
     * (one-indexed). From Mathematica.
     */
