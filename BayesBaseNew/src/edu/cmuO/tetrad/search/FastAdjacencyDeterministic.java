...
/**
 * Implements the "fast adjacency search" for deterministic searches. At a given
 * stage of the search, an edge x *-* y is removed from the graph if S
 * determines either x or y. , where S is a subset of size d either of adj(x) or
 * of adj(y), where d is the depth of the search. The fast adjacency search
 * performs this procedure for each pair of adjacent edges in the graph and for
 * each depth d = 0, 1, 2, ..., d1, where d1 is the first such depth at which no
 ...
 *          2006) $
 */
public class FastAdjacencyDeterministic {
     ...
     */
    public FastAdjacencyDeterministic(Graph graph, IndependenceTest test) {
        this.graph = graph;
        this.test = test;
    }
            ...
            for (Node y : b) {
                ...
                if (ppx.size() >= depth) {
                    ...
                    while ((choice = cg.next()) != null) {
                        ...
                        if (!test.splitDetermines(condSet, x, y)) {
                            if (test.isIndependent(x, y, condSet) &&
                                    noEdgeRequired) {
