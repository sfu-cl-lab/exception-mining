 ...
 * However, it models a general concept: storage for a variety of seperate changes
 * one could possibly apply to a PAG.
 *
 * By convention, the NodePairs in the orients ArrayList are ordered such that Node1
 * is the "from" node and Node2 is the "to" node (ie 1 o-> 2)
 */
public class GraphChange {
     ...
     * Will return null if some change fails (ie an obscure inconsistensy).
     */
    public Pag applyTo (Pag graph) {
        Pag output = new Pag(graph);
        output = makeNewEdges(output);

        for (Triple t : nonColliders)
            output.addUnderlineTriple(t);
    ...
    /**
     * Contains is defined such that if the internal strucs of this GraphChange
     * all individually contain the elements in the corresponding strucs of
     * GraphChange gc, then this "contains" gc.
     */
    ...
    /**
     * Equals is defined such that if the internal strucs of this GraphChange
     * all individually equal the corresponding strucs of GraphChange gc, then
     * this "equals" gc
     */
    @Override
    ...
    /**
     * Takes a graph and recreates all edges. Used in order to copy a graph, because
     * the copy constructor does not go all the way down through the datastructures
     * to make entirely new objects for everything
     */
    private Pag makeNewEdges(Pag graph) {
    ...
    /**
     * Almost a direct copy of edu.cmu.tetrad.graph.NodePair. While  NodePairs can be
     * partially tricked into preserving order of nodes, its .equals does not 
     * acknowledge said convention, thus the need for OrderedNodePairs on occasion
     */
