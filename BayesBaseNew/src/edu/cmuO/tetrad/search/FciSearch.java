...
public final class FciSearch {
     ...
     */
    public FciSearch(IndependenceTest independenceTest, Knowledge knowledge) {
     ...
     */
    public FciSearch(IndependenceTest independenceTest, Knowledge knowledge, List<Node> searchVars) {
        if (independenceTest == null || knowledge == null) {
            throw new NullPointerException();
        }
    ...
    public Pag search() {
        ...
        TetradLogger.getInstance().info("Starting FCI algorithm.");
        ...
        PossibleDSepSearch possibleDSep =
                new PossibleDSepSearch(graph, independenceTest, getSepsetMap());
    ...
    private void orientColliders() {
        ...
        for (Node b : nodes) {
            ...
            while ((combination = cg.next()) != null) {
                ...
                // TODO Why should one need to check that sepset is not null here?

                //if A and C are d-connected conditional on B, and background knowledge
                //does not forbid it, orient as a collider.
//                if (/*sepsetMap.get(A, C) != null &&*/ !sepsetMap.get(A, C).contains(B) &&
//                        isDirEdgeAllowed(A, B) && isDirEdgeAllowed(C, B)) {
...
//                    SearchLogUtils.logColliderOriented(A, B, C, LOGGER);
//                }
                ...
                if (!sepsetMap.get(a, c).contains(b)) {
                    ...
                    graph.setEndpoint(a, b, Endpoint.ARROW);
                    graph.setEndpoint(c, b, Endpoint.ARROW);
                    TetradLogger.getInstance().colliderOriented(SearchLogUtils.colliderOrientedMsg(a, b, c));
                }
            }
        }
    }
    ...
    private void fciOrientbk(Knowledge bk, Graph graph, List<Node> variables) {
        TetradLogger.getInstance().info("Starting BK Orientation.");
        ...
        TetradLogger.getInstance().info("Finishing BK Orientation.");
    }
    ...
    private void doDdpOrientation(Node l, Node a, Node b, Node c) {
        ...
        if (sepset.contains(b)) {
            graph.setEndpoint(c, b, Endpoint.TAIL);
            TetradLogger.getInstance().edgeOriented(SearchLogUtils.edgeOrientedMsg("Definite discriminating path", graph.getEdge(c, b)));
            changeFlag = true;
        }
        else {
            if (!isArrowpointAllowed(a, b)) {
            ...
            graph.setEndpoint(a, b, Endpoint.ARROW);
            graph.setEndpoint(c, b, Endpoint.ARROW);
            TetradLogger.getInstance().colliderOriented(SearchLogUtils.colliderOrientedMsg("Definite discriminate path", a, b, c));
            changeFlag = true;
        }
    }
