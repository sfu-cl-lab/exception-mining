 ...
 * @version $Revision: 6556 $ $Date: 2006-01-20 13:34:55 -0500 (Fri, 20 Jan
 *          2006) $
 */
public class TestFciSearch extends TestCase {
    ...
    public void testSearch1() {
        ...
        checkSearch("X1-->X2,X1-->X3,X2-->X4,X3-->X4",
                "X1o-oX2,X1o-oX3,X2o->X4,X3o->X4", new Knowledge());
    }
    ...
    /**
     * Basic discriminating path checker with an extra variable thrown in. (For
     * some reason, FCI was screwing up on this.)
     */
    ...
    /**
     * A specific graph. This is the graph on p. 137 (2000) or p. 180 (1993) of
     * CPS.
     */
    public void testSearch6() {
        ...
        checkSearch("Latent(E),Latent(G),E-->D,E-->H,G-->H,G-->L,D-->L,D-->M," +
                "H-->M,L-->M,S-->D,I-->S,P-->S",
                "D<->H,H<->L,D-->L,D-->M,H-->M,L-->M,S-->D,Io->S,Po->S", new Knowledge());
    }

    /**
     * A specific graph. This is the graph on p. 5 of Danks, Learning Integrated
     * Structure from Distributed Databases with Overlapping Variables.
     */
    ...
    /**
     * A specific graph. This is the test case from p. 142-144 that tests the
     * possible Dsep step of FCI. This doesn't work in the optimized FCI
     * algorithm. It works in the updated version (FciSearch).  (ekorber)
     */
    public void testSearch8() {
        ...
        checkSearch(
                "Latent(T1),Latent(T2),T1-->A,T1-->B,B-->E,F-->B,C-->F,C-->H," +
                        "H-->D,D-->A,T2-->D,T2-->E",
                "A<->B,B-->E,Fo->B,Fo-oC,Co-oH,Ho->D,D<->E,D-->A", new Knowledge());
    }
...
//        System.out.println(knowledge);
    ...
    public void testMarginal() {
        ...
        Pag G1 = new Pag(Arrays.asList(A, B, C, D));
        ...
        FciSearch fci1 = new FciSearch(new IndTestDSep(G1), new Knowledge());
        System.out.println(fci1.search());
        FciSearch fci2 = new FciSearch(new IndTestDSep(G1), new Knowledge(), Arrays.asList(A, B, C));
        System.out.println(fci2.search());
        FciSearch fci3 = new FciSearch(new IndTestDSep(G1), new Knowledge(), Arrays.asList(A, B, D));
        System.out.println(fci3.search());
        FciSearch fci4 = new FciSearch(new IndTestDSep(G1), new Knowledge(), Arrays.asList(A, D, C));
        System.out.println(fci4.search());
        FciSearch fci5 = new FciSearch(new IndTestDSep(G1), new Knowledge(), Arrays.asList(B, D, C));
    ...
    public void rtestSearch4() {
        ...
        Dag trueGraph = GraphUtils.createRandomDag(numVars, 10, numEdges, 7, 5,
                5, false);
...
//        RectangularDataSet dataSet = bayesIm.simulateData(sampleSize);
        ...
        FciSearch search = new FciSearch(test, new Knowledge());
...
//        RectangularDataSet dataSet = bayesIm.simulateData(sampleSize);
        ...
        System.out.println("\nUsing local test:");
        Cfci search2 = new Cfci(test, new Knowledge());
...
//        System.out.println(resultGraph);
    ...
    public void rtestSearch6() {
        ...
        Dag trueGraph = GraphUtils.createRandomDag(numVars, numLatents, numEdges, 7, 5,
                5, false);
...
//        RectangularDataSet dataSet = bayesIm.simulateData(sampleSize);
        ...
        Cfci search = new Cfci(test, new Knowledge());
    ...
    private void checkSearch(String inputGraph, String outputGraph, Knowledge knowledge) {
        ...
        // Run search
        Pag resultGraph = fciSearch.search();

        // Build comparison graph.
        Pag compareGraph = new Pag(GraphConverter.convert(outputGraph));
        ...
        resultGraph.setUnderLineTriples(compareGraph.getUnderLineTriples());
        resultGraph.setDottedUnderLineTriples(compareGraph.getDottedUnderLineTriples());

        assertTrue(compareGraph.equals(resultGraph));

        // Redo tests for CFCI.
        Cfci cfci = new Cfci(independence, knowledge);
