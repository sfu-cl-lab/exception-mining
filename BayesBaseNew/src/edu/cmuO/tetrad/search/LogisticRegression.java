...
public class LogisticRegression {
     ...
     */
    public String regress(double[] target, String targetName) {
        String report = "";
        ...
        for (int i = 0; i < numCases; i++) {
            ...
            for (int j = 1; j <= numRegressors; j++) {
                //System.out.println("case " + i + " var " + j + " = " + x[j][i]);
        ...
        for (int j = 1; j <= numRegressors; j++) {
            ...
            xStdDevs[j] /= nc;
            xStdDevs[j] =
                    Math.sqrt(Math.abs(xStdDevs[j] - xMeans[j] * xMeans[j]));
        ...
        while (Math.abs(llP - ll) > 0.0000001) {
        ...
        result = new LogisticRegressionResult(targetName,
                ...
                parStdErr, getpValues(), intercept, report
        );
        return report;
    }
