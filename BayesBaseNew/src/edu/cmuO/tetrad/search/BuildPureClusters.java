...
/**
 * BuildPureClusters is a implementation of the automated clustering and
 * purification methods described on the report "Learning Measurement Models"
 * CMU-CALD-03-100.
 * <p/>
 * The output is only the purified model. Future versions may include options to
 * visualize the measurement pattern in the GUI (it shows up in the console
 * window, though.)
 * <p/>
 * No background knowledge is allowed yet. Future versions of this algorithm
 * will include it.
 * <p/>
 * References:
 * <p/>
 * Silva, R.; Scheines, R.; Spirtes, P.; Glymour, C. (2003). "Learning
 * measurement models". Technical report CMU-CALD-03-100, Center for Automated
 * Learning and Discovery, Carnegie Mellon University.
 * <p/>
 ...
 * undirected graph". Communications of ACM 16, 575-577.
 *
 * @author Ricardo Silva
 * @version $Revision: 6124 $ $Date: 2007-08-08 06:27:22 -0400 (Wed, 08 Aug 2007) $
 */
...
public final class BuildPureClusters {
    ...
    /**
     * Information for the Tetrad test
     */
    ...
    // Rearranged these to match the new order in the view --jdramsey 06/14/07
    public static final int PURIFY_TEST_NONE = -1;

    public static final int PURIFY_TEST_GAUSSIAN_PVALUE = 0;
    public static final int PURIFY_TEST_TETRAD_BOLLEN = 1;
    public static final int PURIFY_TEST_TETRAD_WISHART = 2;
    public static final int PURIFY_TEST_GAUSSIAN_SCORE_MARKS = 3;
    public static final int PURIFY_TEST_GAUSSIAN_SCORE = 4;
    public static final int PURIFY_TEST_GAUSSIAN_SCORE_ITERATE = 5;

    public static final int PURIFY_TEST_DISCRETE_LRT = 1;
    public static final int PURIFY_TEST_DISCRETE_VARIATIONAL = 2;

    private int sigTestType, purifyTestType;
    private int labels[];
    ...
    public BuildPureClusters(CovarianceMatrix covarianceMatrix, double sig,
            int sigTestType, int purifyTestType) {
        ...
        initAlgorithm(sig, sigTestType, purifyTestType);
    }

    public BuildPureClusters(CorrelationMatrix correlationMatrix, double sig,
            int sigTestType, int purifyTestType) {
        if (sigTestType == TEST_TETRAD_BOLLEN) {
            throw new RuntimeException(
                    "Covariance Matrix is not enough to run Bollen's tetrad test.");
        }
        this.correlationMatrix = correlationMatrix;
        initAlgorithm(sig, sigTestType, purifyTestType);
    }

    public BuildPureClusters(RectangularDataSet dataSet, double sig,
            int sigTestType, int purifyTestType) {
...
//            this.dataSet = dataSet;
//            assert sigTestType == BuildPureClusters.TEST_DISCRETE;
//            initAlgorithm(sig, sigTestType, purifyTestType);
        }
    }

    private void initAlgorithm(double sig, int sigTestType, int purifyTestType) {

        // Check for missing values.
        if (DataUtils.containsMissingValue(this.correlationMatrix.getMatrix())) {
        ...
        this.scoreTestMode = (this.sigTestType == TEST_DISCRETE ||
                this.sigTestType == TEST_GAUSSIAN_FACTOR);

        if (sigTestType == TEST_DISCRETE) {
            numV = dataSet.getNumColumns();
            indTest = new IndTestGSquare(dataSet, sig);
            tetradTest = new DiscreteTetradTest(dataSet, sig);
        }
        else {
            numV = correlationMatrix.getSize();
            indTest = new IndTestCramerT(correlationMatrix, sig);
            int type = -1;
            switch (sigTestType) {
                case TEST_TETRAD_WISHART:
                    type = ContinuousTetradTest.TEST_TETRAD_WISHART;
                    break;
                case TEST_TETRAD_BOLLEN:
                    type = ContinuousTetradTest.TEST_TETRAD_BOLLEN;
                    break;
                case TEST_GAUSSIAN_FACTOR:
                    type = ContinuousTetradTest.TEST_GAUSSIAN_FACTOR;
                    break;
                default:
                    assert false;
            }
            if (sigTestType == TEST_TETRAD_BOLLEN || purifyTestType == PURIFY_TEST_TETRAD_BOLLEN) {
                tetradTest = new ContinuousTetradTest(dataSet, type, sig);
            }
            else {
                tetradTest = new ContinuousTetradTest(correlationMatrix, type,
                        sig);
            }
        }
    ...
    private void shuffleVariables() {
        ...
        int indices[] = new int[numV];
        for (int i = 0; i < numV; i++) {
            indices[i] = i;
        }
        for (int i = 0; i < numV * 100; i++) {
            int r1 = PersistentRandomUtil.getInstance().nextInt(numV);
            int r2 = PersistentRandomUtil.getInstance().nextInt(numV);
            int temp = indices[r1];
            indices[r1] = indices[r2];
            indices[r2] = temp;
        }
        if (correlationMatrix != null) {
            correlationMatrix = shuffleCorrMatrix(correlationMatrix, indices);
        }
        if (dataSet != null) {
    ...
    private CorrelationMatrix shuffleCorrMatrix(
            CorrelationMatrix originalCorrMatrix, int indices[]) {
        int numV = originalCorrMatrix.getSize();
        String newVarNames[] = new String[numV];
        String originalVarNames[] = originalCorrMatrix.getVariableNames()
                .toArray(new String[0]);
        for (int i = 0; i < numV; i++) {
            newVarNames[i] = originalVarNames[indices[i]];
        }
        double originalData[][] = originalCorrMatrix.getMatrix().toArray();
        double newData[][] = new double[numV][numV];
        for (int i = 0; i < numV; i++) {
            for (int j = i; j < numV; j++) {
                newData[i][j] = newData[j][i] =
                        originalData[indices[i]][indices[j]];
            }
        }
        return new CorrelationMatrix(new CovarianceMatrix(newVarNames, newData,
                originalCorrMatrix.getSampleSize()));
    }

    public static String[] getTestDescriptions() {
        //String testNames[] = new String[3];
        String testNames[] = new String[2];
        testNames[0] = "Wishart's Tetrad Test";
        testNames[1] = "Bollen's Tetrad Test";
        //testNames[2] = new String("Gaussian Factors Test");
        /**
    ...
    public static String[] getPurifyTestDescriptions() {
        ...
        // Rearranged these-- jdramsey 6/14/07

        String testNames[] = new String[5];
        testNames[0] = "Gaussian P-value";
        testNames[1] = "Bollen's Tetrad Test";
        testNames[2] = "Wishart's Tetrad Test";
        testNames[3] = "Gaussian BIC (mark only)";
        testNames[4] = "Gaussian BIC";
        return testNames;
    }
    ...
    public Graph search() {
        ...
        printlnMessage(
                "\n**************Starting search now!!!\n*************\n");
        List clustering = findMeasurementPattern();
        java.util.Date date2 = new java.util.Date();
        printlnMessage("Started at " + date1.toString() + ",finished at " +
                date2.toString());
...
//        }
        ...
        CorrelationMatrix covMatrix = this.correlationMatrix;

        if (graph.getNumNodes() > 1 && covMatrix != null) {
//            MimBuildEstimator estimator = MimBuildEstimator.newInstance(
//                    this.correlationMatrix, new SemPm(graph));
//            estimator.estimate();
            ...
            printlnMessage("chisq = " +
                    estimator.getEstimatedSem().getChiSquare());
            printlnMessage("p-value = " +
    ...
    private Graph convertSearchGraph(List clusters) {
        ...
        if (clusters == null || clusters.size() == 0) {
            ...
            return new EndpointMatrixGraph(nodes);
//            return null;
        }
        ...
        printlnMessage("#ImpurePairs = " + impurePairs.size() / 2);
    ...
    private List convertGraphToList(Graph solutionGraph) {
        impurePairs.clear();
        Iterator it1 = solutionGraph.getNodes().iterator();
        List latentsList = new ArrayList();
        List clusters = new ArrayList();
        while (it1.hasNext()) {
            Node next = (Node) it1.next();
    ...
    private boolean uncorrelatedPopulation(int v1, int v2) {
        return Math.abs(correlationMatrix.getValue(v1, v2)) < 0.0001;
    }

    private boolean vanishingPartialCorrPopulation(int x, int y, int z) {
        double pc = correlationMatrix.getValue(x, y)
                - correlationMatrix.getValue(x, z) *
                correlationMatrix.getValue(y, z);
        pc /= Math.sqrt(1 - correlationMatrix.getValue(x, z) *
                correlationMatrix.getValue(x, z)) *
                Math.sqrt(1 - correlationMatrix.getValue(y, z) *
                correlationMatrix.getValue(y, z));
        double r = 0.5 * Math.sqrt(correlationMatrix.getSampleSize() - 4) *
                Math.log((1. + pc) / (1. - pc));
    ...
    /**
     * Rules to separate x1 and y1, assuming no correlations and partial
     * correlations vanish.
     */

    //private boolean rule1(int x1, int x2, int x3, int y1, int y2, int y3) {
    ...
    //               !tetradTest.oneFactorTest(x1, x2, y1, y2) &&
    //               tetradTest.twoFactorTest(x1, x2, y1, y2);
    //    return
    //        tetradTest.tetradScore3(x1, y1, x2, x3) &&
    ...
    private boolean uncorrelated(int v1, int v2) {
        if (sigTestType == TEST_POPULATION) {
            return uncorrelatedPopulation(v1, v2);
        }
        if (correlationMatrix != null) {

            if (indTest.isIndependent(
                    correlationMatrix.getVariables().get(v1),
                    correlationMatrix.getVariables().get(v2),
                    new ArrayList<Node>())) {
                System.out.println(correlationMatrix.getVariables()
                        .get(v1) + " " +
                        correlationMatrix.getVariables().get(v2) +
                        " == " + correlationMatrix.getValue(v1, v2));
            }

            return indTest.isIndependent(
                    correlationMatrix.getVariables().get(v1),
                    correlationMatrix.getVariables().get(v2),
                    new ArrayList<Node>());
        }
        else {
            return indTest.isIndependent(dataSet.getVariable(v1),
    ...
    private boolean vanishingPartialCorr(int x, int y, int z) {
        ...
        if (correlationMatrix != null) {
            ...
            conditional.add(new ContinuousVariable(
                    correlationMatrix.getVariableName(z)));

            if (!indTest.isIndependent(
                    correlationMatrix.getVariables().get(x),
                    correlationMatrix.getVariables().get(y),
                    conditional)) {
                return indTest.isIndependent(
                        correlationMatrix.getVariables().get(x),
                        correlationMatrix.getVariables().get(y),
            ...
            System.out.println(new StringBuilder().append(correlationMatrix.getVariables()
                    .get(x)).append(" ").append(correlationMatrix.getVariables().get(y)).append(" = ").append(correlationMatrix.getValue(x, y)).append("(").append(correlationMatrix.getVariableName(
                    z)).append(")").toString());
            ...
            return indTest.isIndependent(dataSet.getVariable(x),
    ...
    /**
     * Find components of a graph. Note: naive implementation, but it works.
     * After all, it will still run much faster than Stage 2 of the
     * FindMeasurementPattern algorithm.
     */
    ...
    /**
     * Find all maximal cliques of a graph. However, it can generate an
     * exponential number of cliques as a function of the number of impurities
     * in the true graph. Therefore, we also use a counter to stop the
     * computation after a given number of calls. </p> This is an implementation
     * of Algorithm 2 from Bron and Kerbosch (1973).
     */

    private List findMaximalCliques(int elements[], int ng[][]) {
        boolean connected[][] = new boolean[this.numV][this.numV];
    ...
    private void findMaximalCliquesOperator(int numCalls[], int elements[],
            List output, boolean connected[][], int compsub[], int c[],
    ...
    /**
     * Returns true iff "newClique" is contained in some element of
     * "clustering".
     */
    ...
    /**
     * Transforms clusterings (each "clustering" is a set of "clusters"), remove
     * overlapping indicators for each clustering, and order clusterings
     * according to the number of nonoverlapping indicators, throwing away any
     * latent with zero indicators. Also, for each pair of indicators such that
     * they are linked in ng, one of them is chosen to be removed (the heuristic
     * is, choose the one that belongs to the largest cluster). </p> The list
     * that is returned is a list of lists. Each element in the big list is a
     * list of integer arrays, where each integer array represents one cluster.
     */
    ...
    private List filterAndOrderClusterings(List baseListOfClusterings,
            List baseListOfIds, List clusteringIds, int ng[][]) {
        ...
        for (int i = 0; i < baseListOfClusterings.size(); i++) {
            ...
            List baseIds = (List) baseListOfIds.get(i);
            ...
            boolean impurities[][] = new boolean[this.numV][this.numV];
            ...
            List finalUsedIds = new ArrayList();
            ...
            if (finalNewClustering.size() > 0) {
                ...
                for (int j = 0; j < finalUsedIds.size(); j++) {
                    usedIdsArray[j] =
                            ((Integer) finalUsedIds.get(j)).intValue();
                }
    ...
    private List removeMarkedImpurities(List partition, boolean impurities[][]) {
        ...
        boolean eliminated[] = new boolean[this.numV];
...
//        }
    ...
    private List initialMeasurementPattern(int ng[][], int cv[][]) {
        ...
        boolean notYellow[][] = new boolean[numV][numV];
        ...
        for (int i = 0; i < numV; i++) {
            for (int j = i + 1; j < numV; j++) {
                if (cv[i][j] == EDGE_NONE) {
                    printlnMessage(tetradTest.getVarNames()[i] + " || " +
        ...
        for (int v1 = 0; v1 < numV - 1; v1++) {
            for (int v2 = v1 + 1; v2 < numV; v2++) {
                ...
                if (notFound) {
                    ...
                    printlnMessage("Pairwise test: " + tetradTest.getVarNames()[v1] +
                            " x " +
        ...
        for (int v1 = 0; v1 < numV - 1; v1++) {
            for (int v2 = v1 + 1; v2 < numV; v2++) {
                ...
                for (int v3 = 0; v3 < numV - 1 && notFound; v3++) {
                    ...
                    for (int v5 = v3 + 1; v5 < numV && notFound; v5++) {
                        ...
                        for (int v4 = 0; v4 < numV - 1 && notFound; v4++) {
                ...
                if (notFound) {
                    ...
                    for (int v3 = 0; v3 < numV && notFound; v3++) {
                        ...
                        for (int v4 = 0; v4 < numV - 2 && notFound; v4++) {
        ...
        List components = findComponents(ng, numV, EDGE_BLUE);
        ...
        while (it.hasNext()) {
            ...
            List nextClustering = findMaximalCliques(component, ng);
            System.out.println(
        ...
        List individualOneFactors = individualPurification(clustering);
    ...
    private List individualPurification(List clustering) {
        ...
        for (int i = 0; i < clustering.size(); i++) {
            ...
            dummyIds.add(ids);
            List purification = purify(dummyClusterings, dummyIds, null);
    ...
    private List findMeasurementPattern() {
        int ng[][] = new int[numV][numV];
        int cv[][] = new int[numV][numV];
        boolean selected[] = new boolean[numV];
        for (int i = 0; i < numV; i++) {
            selected[i] = false;
        }
        ...
        List forbiddenList = new ArrayList();
    ...
    private List chooseClusterings(List clustering, int ng[][], List outputIds,
            boolean need3, int cv[][]) {
        List clusterings = new ArrayList();
        boolean marked[] = new boolean[clustering.size()];
        boolean buffer[] = new boolean[this.numV];
        ...
        for (int i = 0; i < max; i++) {
            ...
            //printClusterNames((int[]) clustering.get(i));
            int bestChoice;
            ...
            do {
                ...
                for (int j = 0; j < clustering.size(); j++) {
                    ...
                    //printClusterNames((int[]) clustering.get(j));
            ...
            //System.out.println("-- New candidate clustering");
            //printClustering(newClustering);
    ...
    /**
     * Score a pair of clusters according to the non-overlapping nodes that are
     * not linked by any edge in ng.
     */
    ...
    /**
     * Mark all elements that show up in 'newCluster' and also in some element
     * in 'clustering'. Arrays 'toRemove' and 'usedNodes' will be updated.
     *
     * @param clustering
     * @param newCluster
     * @param toRemove
    ...
    /**
     * Remove from any element of 'clustering' all nodes that are marked to be
     * removed.
     *
     * @param clustering
     * @param toRemove
    ...
    private List purify(List actualClusterings, List clusterIds,
            List forbiddenList) {
        printlnMessage();
        printlnMessage("** PURIFY: total number of clustering candidates = " +
        ...
        for (int i = 0; i < actualClusterings.size() && i < 10; i++) {
            ...
            printlnMessage("Remaining: " + (actualClusterings.size() - i - 1));
            ...
            int constraintSearchVariation = 1;
            int oldPurifyTest = -1;
            if (!(tetradTest instanceof DiscreteTetradTest)) {
                oldPurifyTest =
                        ((ContinuousTetradTest) tetradTest).getTestType();
                switch (this.purifyTestType) {
                    case PURIFY_TEST_GAUSSIAN_SCORE:
                        ((ContinuousTetradTest) tetradTest).setTestType(
                                ContinuousTetradTest.TEST_GAUSSIAN_SCORE);
                        break;
                    case PURIFY_TEST_GAUSSIAN_SCORE_MARKS:
                        ((ContinuousTetradTest) tetradTest).setTestType(
                                ContinuousTetradTest.TEST_GAUSSIAN_SCORE_MARKS);
                        break;
                    case PURIFY_TEST_GAUSSIAN_SCORE_ITERATE:
                        ((ContinuousTetradTest) tetradTest).setTestType(
                                ContinuousTetradTest.TEST_GAUSSIAN_SCORE_ITERATE);
                        break;
                    case PURIFY_TEST_TETRAD_WISHART:
                        ((ContinuousTetradTest) tetradTest).setTestType(
                                ContinuousTetradTest.TEST_TETRAD_WISHART);
                        break;
                    case PURIFY_TEST_TETRAD_BOLLEN:
                        ((ContinuousTetradTest) tetradTest).setTestType(
                                ContinuousTetradTest.TEST_TETRAD_BOLLEN);
                        break;
                    case PURIFY_TEST_GAUSSIAN_PVALUE:
                        ((ContinuousTetradTest) tetradTest).setTestType(
                                ContinuousTetradTest.TEST_GAUSSIAN_PVALUE);
                        break;
                    case PURIFY_TEST_NONE:
                        ((ContinuousTetradTest) tetradTest).setTestType(
                                ContinuousTetradTest.TEST_NONE);
                        break;
//                        case BuildPureClusters.PURIFY_TEST_TETRAD_BOLLEN2:
//                            ((ContinuousTetradTest) tetradTest).setIndTestType(ContinuousTetradTest.TEST_TETRAD_BOLLEN);
//                        	constraintSearchVariation = 1;
//                            break;
                    default:
                        assert false;
                }
            }
            Purify purifier = new Purify(tetradTest, knowledge);
    ...
    /**
     * Given a set of clusters, substitute it by n sets of size n-1 in the
     * cliqueList. This is a heuristic for resusing a set of latents that failed
     * purification even though they form a clique. For a set of n latents, we
     * generate n sets where in each one we copy n-1 elements of the original
     * set. Needless to say, this has an exponential growth.
     */
