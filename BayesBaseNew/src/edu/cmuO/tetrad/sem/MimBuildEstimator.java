 ...
 * estimates of the parameters of a linear, Gaussian SEM for a given covariance
 * matrix; it contains member variables for storing the results of such a
 * computation. The values of the parameters are a search procedure based on
 ...
 * @version $Revision: 6379 $ $Date: 2006-01-08 23:54:59 -0500 (Sun, 08 Jan
 *          2006) $
 */
...
public class MimBuildEstimator {
    ...
    private RectangularDataSet dataSet = null;
    ...
    /**
     * A maximum likelihood estimate of the parameters of a SEM is done with
     * respect to a sample.  The constructor therefore takes a SEM and a
     * covariance matrix as arguments.
     */

    public MimBuildEstimator(CovarianceMatrix covMatrix, SemPm semPm,
    ...
    public static MimBuildEstimator newInstance(RectangularDataSet dataSet,
            SemPm semPm) {
    ...
    public static MimBuildEstimator newInstance(CovarianceMatrix covMatrix,
            SemPm semPm) {
    ...
    private void buildMeasurementStructuralModel() {
        ...
        numLatents = numFixedIndicators = latentsL.size();
    ...
    private void randomizeParameters() {
        for (int i = 0; i < theta.length; i++) {
            if (i < indicatorErrorsIndex) {
                ...
                theta[i] =
                        PersistentRandomUtil.getInstance().nextDouble() + 0.1;
            }
            //theta[i] = 1.;
        }
    }
    ...
    private void randomizeVariancesOnly() {
        for (int i = indicatorErrorsIndex; i < theta.length; i++) {
            theta[i] = PersistentRandomUtil.getInstance().nextDouble() + 0.1;
        }
    }
     ...
     */
    private CovarianceMatrix fixVarOrder(SemPm semPm,
    ...
    private SemIm getOptimizedSem() {
        //First, create a copy of semPm where the proper parameters
        //are fixed
        SemPm fixedPm;
        try {
...
//                    Iterator<Node> itChild = semIm.getSemPm().getGraph()
