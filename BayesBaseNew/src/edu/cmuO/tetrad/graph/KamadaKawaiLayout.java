 ...
 * @version $Revision: 6231 $ $Date: 2006-01-11 13:36:55 -0500 (Wed, 11 Jan
 *          2006) $
 */
public final class KamadaKawaiLayout {
     ...
     * should be initialized in their current locations.
     */
    ...
    private void initialize(List<Node> nodes, boolean randomlyInitialized) {
        ...
        if (randomlyInitialized) {
            for (int i = 0; i < nodes.size(); i++) {
                p[i][0] = PersistentRandomUtil.getInstance().nextInt(600);
                p[i][1] = PersistentRandomUtil.getInstance().nextInt(600);
            }
        }
        else {
            for (int i = 0; i < nodes.size(); i++) {
    ...
    private void optimize(double deltaCutoff) {
        ...
        do {
            ...
            if (m[0] == oldM) {
                p[m[0]][0] += PersistentRandomUtil.getInstance().nextInt(
                        2 * jump) - jump;
                p[m[0]][1] += PersistentRandomUtil.getInstance().nextInt(
                        2 * jump) - jump;
                continue;
            }
