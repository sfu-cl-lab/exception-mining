...
/**
 * Represents an ancestral graph--that is, a graph with the following graph
 * constraints: <ol> <li> Four basic edge types (from Graph) <li> edges unique
 * up to type (from Graph) <li> Observed and latent variables only. <li> No
 * edges of the form Xo->Y.  ("No partially oriented edges.") <li> If X->Y, then
 * Y is not an ancestor of X.  ("Arrow implies non-ancestor.") <li> If X--Y,
 * then there cannot be any edges into X or Y.  ("Undirected edges block into
 * edges.") </ol>
 *
 ...
 *          2006) $
 */
public final class AncestralGraph implements TetradSerializable, Graph {
    static final long serialVersionUID = 23L;

    private final static GraphConstraint[] constraints = {
            new MeasuredLatentOnly(), new NoHalfdirectedEdges(),
            new AtMostOneEdgePerPair(), new NoEdgesToSelf(),
            new InArrowImpliesNonancestor(),
            new NoEdgeIntoUndirectedEndpoint()};
    ...
    /**
     * Constructs a new empty ancestral graph.
     */
    public AncestralGraph() {
    ...
    /**
     * Constructs a new ancestral graph based on a given Graph.
     *
     * @param graph The graph to construct an ancestral graph from.
     * @throws IllegalArgumentException if the given graph cannot be converted
     *                                  to an AncestralGraph.
     */
    public AncestralGraph(Graph graph) throws IllegalArgumentException {
        List<GraphConstraint> constraints1 = Arrays.asList(constraints);
     ...
     */
    public static AncestralGraph serializableInstance() {
        return new AncestralGraph();
    }
	...
	public final void transferNodesAndEdges(Graph graph)
        ...
        graph.transferNodesAndEdges(graph);
    }
