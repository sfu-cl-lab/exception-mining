...
import edu.cmu.tetrad.search.IndTestSepset;
...
public class Dci {
    ...
    /**
     * Keeps up with current graph while searching for possible unshielded sets and
     */
    ...
    public List<Graph> search() {
        ...
        // find set of minmal spanning treks and for each trek, its ensuring paths
    ...
    /**
     * Finds the discriminating paths relative only to variables measured jointly after the initial definite colliders
     * have been oriented.
     * <p/>
     ...
     * a DDP consists of colliders that are parents of c. This only considers discriminating paths that are completely
     * jointly measured.
     */
    ...
    private class FindMinimalSpanningTrek implements Runnable {
        ...
        public void run() {
            while (currentThread < totalThreads) {
                ...
                for (List<Node> trek : GraphUtils.treks(currentGraph, currentGraph.getNode(nodePair.getFirst().getName()), currentGraph.getNode(nodePair.getSecond().getName()))) {
...
//                  allPaths.get(trek.size()).put(trek, otherNodes);
     ...
     * the sequence mapped to ancestral paths
     */
    ...
    /**
     * For each necessary trek in a marginal graph, finds the treks in the current graph that traverse each node in the
