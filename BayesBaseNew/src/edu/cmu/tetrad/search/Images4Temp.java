...
public final class Images4Temp implements GraphSearch, IImages {

    /**
     * The data set, various variable subsets of which are to be scored.
     */
    private List<DataSet> timeSeriesDataSets;
...
//    private double fCutoffP = 0.01;
    ...
    public Images4Temp(List<DataSet> dataSets) {
        ...
        for (DataModel dataModel : dataSets) {
            ...
            DataSet timeSeries = TimeSeriesUtils.createLagData(dataSet, 1);
        ...
        setDataSets(dataSets, timeSeriesDataSets);
    }
...
//                    " (" + nf.format(score)  + ", " + bump + ") " + label);
    ...
    private void setDataSets(List<DataSet> dataSets, List<DataSet> timeSeriesDataSets) {
        ...
        this.discrete = timeSeriesDataSets.get(0).isDiscrete();

        if (!isDiscrete()) {
            this.covariances = new ArrayList<DoubleMatrix2D>();

            for (int i = 0; i < dataSets.size(); i++) {
                this.covariances.add(timeSeriesDataSets.get(i).getCovarianceMatrix());
            }
        }
    ...
    private double localSemScore(int i, int[] parents) {
        ...
        return sum / (double) dataSets().size();
    }

    private double localSemScoreOneDataSet(int dataIndex, int i, int[] _parents) {
        DataSet dataSet = timeSeriesDataSets.get(dataIndex);

        int[] parents = new int[2 * _parents.length];

        for (int h = 0; h < _parents.length; h++) {
            parents[h] = _parents[h];
            parents[h + _parents.length] = _parents[h] + variables.size();
        }
        ...
        if (missingVariables.get(dataSets.get(dataIndex)).contains(dataSets.get(dataIndex).getVariable(i))) {
            return 0;
...
//                inverse = MatrixUtils.ginverse(Czz);
                ...
                for (int j = 0; j < parents.length; j++) {
                    buf.append(timeSeriesDataSets.get(0).getVariables().get(parents[j]));
...
//            System.out.println("Storing " + score + " " + graphCopy);
    ...
    public Map<Edge, Integer> getBoostrapCounts(int numBootstraps) {
        ...
        return bootstrapImagesCounts(timeSeriesDataSets, returnGraph.getNodes(), getKnowledge(), numBootstraps, getPenaltyDiscount());
    ...
    public Map<Edge, Double> averageStandardizedCoefficients(Graph graph) {
        ...
        for (DataSet dataSet : timeSeriesDataSets) {
        ...
        for (Edge edge : coefs.keySet()) {
            coefs.put(edge, coefs.get(edge) / (double) timeSeriesDataSets.size());
        }
