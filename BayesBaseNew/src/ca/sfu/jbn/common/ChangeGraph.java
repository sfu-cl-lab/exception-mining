        ...
        if (!node.getName().startsWith("B(")) {
            ...
            if (graph.getParents(node) != null) {
                for (int j = 0; j < (graph.getParents(node)).size(); j++) {
                    if (!((Node) graph.getParents(node).get(j)).getName().startsWith("B(")) {
                        int indexOfParent = (((Node) graph.getParents(node).get(j)).getName()).indexOf('.');
                        String nameOfParentEntity = (((Node) graph.getParents(node).get(j)).getName()).substring(0, indexOfParent);
