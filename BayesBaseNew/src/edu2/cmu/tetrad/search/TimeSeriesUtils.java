...
public class TimeSeriesUtils {
    ...
    /**
     * Returns the VAR residuals of the given time series with the given number of lags. That is, ever variable at the
     * current lag is regressed onto every variable at previous lags, up to the given number of lags, and the residuals
...
//        Regression regression = new RegressionDatasetGeneralized(timeLags);
    ...
    public static DataSet ar3(DataSet timeSeries) {
        DoubleMatrix2D _timeSeries = timeSeries.getDoubleData();

        DoubleMatrix2D residuals = new DenseDoubleMatrix2D(_timeSeries.rows() - 1,
                timeSeries.getNumColumns());

        for (int j = 0; j < _timeSeries.columns(); j++) {
            DoubleMatrix2D x = _timeSeries.viewPart(1, j, _timeSeries.rows() - 1, 1);
            DoubleMatrix2D y = _timeSeries.viewPart(0, 0, _timeSeries.rows() - 1, _timeSeries.columns());
            ...
            residuals.viewColumn(j).assign(res.viewColumn(0));
        }
...
//        Cpc search = new Cpc(test);
    ...
    public static DataSet createShiftedData(DataSet data, int[] shifts) {
        ...
        DoubleMatrix2D shiftedData = new DenseDoubleMatrix2D(shiftedDataLength, data2.columns());
