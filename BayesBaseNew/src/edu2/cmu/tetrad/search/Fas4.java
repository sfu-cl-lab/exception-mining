...
import edu.cmu.tetrad.search.mb.TestMarkovBlanketSearches;
...
//    private List<Double> pValues = new ArrayList<Double>();
     ...
     */
    public Fas4(Graph graph, IndependenceTest test) {
        this.graph = graph;
        this.test = test;
    }
...
//            if ((i + 1) % 100 == 0) System.out.println("count " + (i + 1) + " of " + nodes.size() + " depth 0" + "**");
            ...
            for (int j = i + 1; j < nodes.size(); j++) {
                ...
                try {
                    independent = test.isIndependent(x, y, empty)/* || test.isIndependent(y, x, empty)*/;
                } catch (Exception e) {
                    e.printStackTrace();
                    independent = false;
                }
...
//                    System.out.println("Leaving " + x + " " + y + " out.");
                    removed++;
                } else {
//                    System.out.println("Putting " + x + " " + y + " in.");
