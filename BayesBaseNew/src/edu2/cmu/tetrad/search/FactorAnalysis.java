...
// method-specific fields that get used
    ...
    */
     ...
     * END PSEUDO-CODE
     *
     * At the end of the method, the list of column vectors is actually assembled into a DenseDoubleMatrix2D.
     *
     */
    ...
    public DenseDoubleMatrix2D successiveResidual() {
        ...
        this.residualMatrices.add((DenseDoubleMatrix2D) correlationMatrix.getMatrix());
        ...
        while (vectorSum(dValues) / trace((DenseDoubleMatrix2D) correlationMatrix.getMatrix()) < .99) {
     ...
     * and returns its current approximation.
     *
    ...
    public static DoubleMatrix2D matrixMult(DoubleMatrix2D a, DoubleMatrix2D b) {
        ...
        DoubleMatrix2D result = new DenseDoubleMatrix2D(a.rows(), b.columns());
    ...
    public static DoubleMatrix2D normalizeRows(DoubleMatrix2D matrix) {
        ...
        DoubleMatrix2D result = new DenseDoubleMatrix2D(matrix.rows(), matrix.columns());
        for (int i = 0; i < matrix.rows(); i++) {
            DoubleMatrix2D normalizedRow = normalizedRows.get(i);
            for (int j = 0; j < matrix.columns(); j++) {
    ...
    public static DoubleMatrix2D successiveFactorVarimax(DoubleMatrix2D factorLoadingMatrix) {
        ...
        DoubleMatrix2D sumCols = matrixMult(transpose(residuals.lastElement()), unitColumn);
        DoubleMatrix2D wVector = matrixDiv(Math.sqrt(matrixMult(matrixMult(transpose(unitColumn), residuals.lastElement()), sumCols).get(0, 0)),
                sumCols);
        ...
        */
        ...
        DoubleMatrix2D vVector = matrixMult(residuals.lastElement(), wVector);
        ...
        for (int k = 0; k < normalizedFactorLoadings.columns(); k++) {
            ...
            for (int i = 0; i < 50; i++) {
                DoubleMatrix2D bVector = matrixMult(residuals.lastElement(), hVectors.get(i));
                ...
                DoubleMatrix2D betaVector = matrixSubtract(matrixExp(bVector, 3), matrixMult(averageSumSquaresBVector, bVector));
                DoubleMatrix2D uVector = matrixMult(transpose(residuals.lastElement()), betaVector);
