...
public final class PValueImprover3 {
    ...
    public PValueImprover3(Graph graph, DataSet data) {
...
//        removeHighPValueEdges(bestGraph);
        increaseScoreLoop(bestGraph, meekRules, .99);
...
//                meekRules.orientImplied(getGraph());
                ...
                if (graph.existsDirectedCycle()) { //SearchGraphUtils.dagFromPattern(getGraph()).existsDirectedCycle()) {
                    continue;
                }

                Graph dag = getGraph(); //SearchGraphUtils.dagFromPattern(getGraph(), getKnowledge());
...
//                meekRules.orientImplied(getGraph());
                ...
                if (getGraph().existsDirectedCycle()) { //SearchGraphUtils.dagFromPattern(getGraph()).existsDirectedCycle()) {
                    continue;
                }
...
//
//                if (_nodes.size() >= 2) {
//                    ChoiceGenerator gen = new ChoiceGenerator(_nodes.size(), 2);
//                    int[] choice;
//
...
//        }
    ...
    private static class Move {
        public enum Type {
            ADD, REMOVE, REDIRECT, ADD_COLLIDER, REMOVE_COLLIDER, COLLIDER_TRIANGLE;
        }
