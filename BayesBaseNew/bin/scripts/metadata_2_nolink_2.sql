...
create view Test2 as select * from FNodes;
insert into FNodes
SELECT 
    1nid AS Fid,
...
/*CREATE TABLE IF NOT EXISTS Knowledge_Required_Edges like Path_BayesNets;*//*Jun 25 we create this table inside BayesBaseCT_SortMege in order to use the general bayesbnets as background knowledge*/
...
CREATE TABLE  RNodeEdges like Path_BayesNets;
CREATE TABLE  InheritedEdges like Path_BayesNets;
CREATE TABLE NewLearnedEdges LIKE Path_BayesNets;
CREATE TABLE Test LIKE Path_BayesNets;


/*INSERT IGNORE INTO Path_BayesNets SELECT * FROM ContextEdges;*/
/* for each rchain, for each edge that was learned in that rchain, make the members of the rchain parents of the tail of the edge.
 * Such edges we call context edges. Later we propagate these up the rchain lattice, but we need to use a program for that.*/
