...
//            initAlgorithm(alpha, sigTestType, purifyTestType);
    ...
    private void initAlgorithm(double alpha, TestType sigTestType, TestType purifyTestType) {
        ...
        if (DataUtils.containsMissingValue(getCovarianceMatrix().getMatrix())) {
...
//        shuffleVariables();
        ...
        if (sigTestType == TestType.DISCRETE) {
            ...
            if (sigTestType == TestType.TETRAD_WISHART || sigTestType == TestType.TETRAD_BOLLEN
                    || sigTestType == TestType.GAUSSIAN_FACTOR) {
                type = sigTestType;
            } else {
                throw new IllegalArgumentException("Expecting TETRAD_WISHART, TETRAD_BOLLEN, or GAUSSIAN FACTOR " +
                        sigTestType);
            }

//            if (sigTestType == TestType.TETRAD_BOLLEN || purifyTestType == TestType.TETRAD_BOLLEN) {
...
//    }
    ...
    public Graph search() {
        ...
        print("Started at " + start.toString() + ", finished at " +
                end.toString());
...
//        }
    ...
    private List initialMeasurementPattern(int ng[][], int cv[][]) {
        ...
        List components = findComponents(ng, numVariables(), EDGE_BLUE);
...
//            IPurify purify = new PurifyTetradBasedD(tetradTest);
