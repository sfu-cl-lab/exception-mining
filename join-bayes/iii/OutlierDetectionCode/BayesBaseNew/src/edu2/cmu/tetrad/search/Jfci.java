...
public class Jfci implements GraphSearch {
     ...
     */
    public Jfci(IndependenceTest independenceTest) {
...
//        System.out.println("Degree of graph = " + GraphUtils.getDegree(graph));
...
//            System.out.println("Num removed = " + numRemoved);
    ...
    private Graph orient(Graph graph, SepsetMap sepsets) {
        return orientPc(graph, knowledge, sepsets);
    }
...
//                if (node1 == x || node1 == y) continue;
    ...
    private Graph orientCpc(Graph graph, Knowledge knowledge, int depth, IndependenceTest test) {
    ...
    private Graph orientPc(Graph graph, Knowledge knowledge, SepsetMap sepsets) {
        graph = GraphUtils.undirectedGraph(graph);
        SearchGraphUtils.pcOrientbk(knowledge, graph, graph.getNodes());
        SearchGraphUtils.orientCollidersUsingSepsets(sepsets, knowledge, graph);
        MeekRules meekRules = new MeekRules();
        meekRules.setAggressivelyPreventCycles(isAggressivelyPreventCycles());
        meekRules.setKnowledge(knowledge);
        meekRules.orientImplied(graph);
