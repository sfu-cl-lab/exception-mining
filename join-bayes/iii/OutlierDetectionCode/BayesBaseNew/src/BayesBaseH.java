...
//	static String orig="Premier_League_2011";
	static String opt3, cont;
	static boolean linkAnalysis;
	//static String backGround="Premier_League_2011_BN";
...
*/		//parameter learning 
		...
		if (!cont.equals("1")) {
			...
			System.out.println("\n Structure Learning is DONE.  Now doing the parameter learning. "); //@zqian	
		...
		//	Pruning();
			...
			// Export to .mln file, Feb 11, 2014
	...
	/*    st.execute("create or replace view LearnedEdges as  select distinct" +
	    		"   Path_BayesNets.Rchain,  Path_BayesNets.child,  Path_BayesNets.parent from  " +
	    		" Path_BayesNets,    lattice_set,    lattice_rel where    Path_BayesNets.parent <> ''  " +
	    		"and lattice_set.name = lattice_rel.parent " +
	    				" (Path_BayesNets.Rchain , Path_BayesNets.child, Path_BayesNets.parent) not " +
	    				"in (select  *  from  Path_Required_Edges);"); 
	    
	    st.execute("create table ContextEdges as select distinct LearnedEdges.Rchain as Rchain," +
	...
	//	st.execute("insert ignore into Path_Required_Edges select * from "+backGround+".Path_BayesNets");
	    ...
	    st.execute("insert ignore into Path_Forbidden_Edges select distinct RNodes_pvars.rnid AS Rchain, Entity_Complement_Edges.child AS child," +
	    		" Entity_Complement_Edges.parent AS parent FROM (RNodes_pvars, Entity_Complement_Edges)   WHERE  " +
	    		"(RNodes_pvars.pvid = Entity_Complement_Edges.pvid)  and (RNodes_pvars.rnid,Entity_Complement_Edges.child, Entity_Complement_Edges.parent) not in (select Rchain, child, parent from Path_Required_Edges);");// To avoid conflicts between Path_RequiredEdges and Path_ForbiddenEdges that may happen after adding Background knowledge Aug2014
	    st.close();
...
//Nov25 
	...
	//####      Design Four: Do NOT differenciated Nodes Type, consider them(1Nodes,2Nodes,RNodes) as the same
	    	...
	    	//with link analysis off, we add the context edges at the end.
	    	/*Statement st = con2.createStatement();
...
public static void PropagateContextEdges() throws Exception {
         ...
         System.out.println("create table ContextEdges as select distinct NewLearnedEdges.Rchain as Rchain," +
                    " NewLearnedEdges.child as child,lattice_membership.member as parent from NewLearnedEdges," +
                    "lattice_membership where NewLearnedEdges.Rchain = lattice_membership.name union select distinct * from Knowledge_Required_Edges where Knowledge_Required_Edges.parent in (select rnid from RNodes) union (select * from SchemaEdges);");
         st.execute( "create table ContextEdges as select distinct NewLearnedEdges.Rchain as Rchain," +
                    " NewLearnedEdges.child as child,lattice_membership.member as parent from NewLearnedEdges," +
                    "lattice_membership where NewLearnedEdges.Rchain = lattice_membership.name union select distinct Rchain, child, parent from Knowledge_Required_Edges where Knowledge_Required_Edges.parent in (select rnid from RNodes) union (select * from SchemaEdges);");//When we add background knowledge in requiredEdges,
         
         // tetrad removes the edges that thinks it is not relevent to this particular individuals, those edges are required but get removed by tetrad! we force add them here! Aug 8 Sarah
      ...
      // `a,b` as rchain, `b` as child, '' as parent
        ...
        Statement st_temp = con2.createStatement();
         for(String id : rnode_ids_1)  {//Feb 7th 2014, zqian; updated May 26, 2014 zqian
            ...
            st_temp.execute("insert into Path_BayesNets (select '"+ largest_rchain +"' as Rchain, '"+id+"'  as child, ''  as parent ) ;");
            //System.out.println ("insert into Path_BayesNets (select '"+ largest_rchain +"' as Rchain, '"+id+"'  as child, ''  as parent ) ;");
         }
            st_temp.close();
