...
public class BayesBaseCT_SortMerge {
	...
	static String databaseName, databaseName2, databaseName3, databaseName4, orignialDB,orig;
	static String dbbase;
	static String dbUsername;
	static String dbPassword;
	static String dbaddress;
	static String opt2, cont,background,GeneralPathBayesNets;
//special parameters for `premier_league`, June 2014
	static String indi;
	static String neg;
...
//
	...
	public static void buildCT() throws Exception {
		...
		System.out.println("dbbase:"+dbbase);
       BZScriptRunner bzsr = new BZScriptRunner(orig,con1);
...
*/
		...
		if(background.equals("1")){
			System.out.println("insert into "+databaseName2 +".Knowledge_Required_Edges select * from "+GeneralPathBayesNets +".Path_BayesNets");
			st.execute("create table if not exists "+databaseName2 +".Knowledge_Required_Edges like "+GeneralPathBayesNets +".Path_BayesNets");
			st.execute("insert into "+databaseName2 +".Knowledge_Required_Edges select * from "+GeneralPathBayesNets +".Path_BayesNets");
		...
		if(indi.equals("0")) {
			...
			if (cont.equals("1")) {
				...
				bzsr.runScript("src/scripts/metadata_2_2.sql");
				} else {
					System.out.println("Running src/scripts/metadata_2_nolink_2.sql");
				bzsr.runScript("src/scripts/metadata_2_nolink_2.sql");
				}
	...
	public static int buildCTTarget() throws Exception {
		...
		connectDB();
		//build _BN part1 from metadata_1.sql
       BZScriptRunner bzsr = new BZScriptRunner(orig,dbbase,con4);
...
*/
	...
	public static void setVarsFromConfig(){
		...
		databaseName4 =orig + "_setup";
	}

	public static void connectDB() throws SQLException {
		String CONN_STR1 = "jdbc:" + dbaddress + "/" + databaseName2 ;
		try {
			...
			if ( null != rsGrounding )
			{
				if ( rsGrounding.absolute(1) )
				{
					...
					whereString += " where " + pvid + "." + 
								   pvidActualId + " = " + 
								   rsGrounding.getString(1);
		/* // directly replace pvid with the actual pvid, June 14
	// the following works fine for `premier_league` database.
			whereString += " where " + pvid + "." + 
								   pvid.replace("s0","") + "ID = " + 
								   rsGrounding.getString(1);*/
