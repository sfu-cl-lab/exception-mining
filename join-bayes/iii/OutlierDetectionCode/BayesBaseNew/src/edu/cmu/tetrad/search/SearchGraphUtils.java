...
//    }
    ...
    public static void basicPattern(Graph graph) {
        ...
        for (Edge edge : graph.getEdges()) {
            ...
            if (head != null) {
                ...
                NEXT_PARENT:
                for (Node parent : graph.getParents(head)) {
                    if (parent != tail) {
                        if (graph.isAdjacentTo(parent, tail)) {
    ...
    public static Graph dagFromPattern(Graph pattern) {
        ...
        pattern = new EdgeListGraph(pattern);
...
//        return GraphUtils.dagFromPattern(pattern, new IKnowledge());
//
...
//        Set<Set<Node>> sepsets = new HashSet<Set<Node>>();
...
//            CHOICE:
            while ((choice = cg.next()) != null) {
                List<Node> condSet = GraphUtils.asList(choice, _nodes);
//                for (Set<Node> _sepset : sepsets) {
//                    if (_sepset.equals(new HashSet<Node>(condSet))) {
//                        continue CHOICE;
//                    }
//                }

                if (test.isIndependent(x, z, condSet)) {
//                    sepsets.add(new HashSet<Node>(condSet));
    ...
    public static CpcTripleType getCpcTripleType2(Node x, Node y, Node z,
        ...
        if (numSepsetsContainingY > factor * numSepsetsNotContainingY) {
            ...
            return CpcTripleType.COLLIDER;
//        } else if (numSepsetsContainingY != 0 && numSepsetsNotContainingY == 0) {
//            return CpcTripleType.NONCOLLIDER;
//        } else if (numSepsetsNotContainingY != 0 && numSepsetsContainingY == 0) {
//            return CpcTripleType.COLLIDER;
        } else {
            return CpcTripleType.AMBIGUOUS;
        }

//        if (numSepsetsContainingY == numSepsetsNotContainingY) {
//            return CpcTripleType.AMBIGUOUS;
//        }
//        else if (!numSepsetsNotContainingY) {
//            return CpcTripleType.NONCOLLIDER;
//        }
//        else {
//            return CpcTripleType.COLLIDER;
