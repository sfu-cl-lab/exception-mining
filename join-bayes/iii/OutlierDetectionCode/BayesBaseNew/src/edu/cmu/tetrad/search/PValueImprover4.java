...
public final class PValueImprover4 implements PValueImprover {
    ...
    public PValueImprover4(Graph graph, DataSet data, Knowledge knowledge) {
    ...
    public PValueImprover4(Graph graph, CovarianceMatrix cov, Knowledge knowledge) {
...
//                    System.out.println("Score " + _score.getPValue() + " DOF = " + _score.getDof());
        ...
        System.out.println("DDOOFF = " + scoreGraph(maximumScore(S)).getDof());
    ...
    public Graph removeZeroEdges(Graph bestGraph) {
        ...
        while (changed) {
            ...
            for (Parameter param : estSem.getSemPm().getParameters()) {
                ...
                if (getKnowledge().edgeRequired(edge.getNode1().getName(), edge.getNode2().getName())) {
...
//                System.out.println("P value for edge " + edge + " = " + p);
    ...
    private List<Move> getAddMoves(Graph graph) {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            for (int j = 0; j < nodes.size(); j++) {
                ...
                if (getKnowledge().edgeRequired(nodes.get(j).getName(), nodes.get(i).getName())) {
                    continue;
                }
    ...
    private List<Move> getRemoveMoves(Graph graph) {
        ...
        for (Edge edge : edges) {
            ...
            if (getKnowledge().edgeRequired(i.getName(), j.getName())) {
                continue;
            }
    ...
    private List<Move> getRedirectMoves(Graph graph) {
        ...
        for (Edge edge : edges) {
            Node i = edge.getNode1();
            Node j = edge.getNode2();
            if (knowledge.edgeForbidden(j.getName(), i.getName())) {
                continue;
            }
