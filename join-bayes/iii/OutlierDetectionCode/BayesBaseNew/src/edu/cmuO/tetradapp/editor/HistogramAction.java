 ...
 *
 * @author Tyler Gibson
 * @version $Revision: 1.1 $ $Date: Nov 28, 2006 12:49:13 AM $
 */
public class HistogramAction extends AbstractAction {
	...
	public void actionPerformed(ActionEvent e) {
        ...
        if (selected != null && 1 < selected.length) {
            ...
            for (int index : selected) {
                JDialog dialog = createHistogramDialog(dataSet.getVariable(index));
                dialog.pack();
                setLocation(dialog, index);
                dialog.setVisible(true);
            }
        } else {
            JDialog dialog = createHistogramDialog(null);
            dialog.pack();
     ...
     */
    private JDialog createHistogramDialog(Node selected) {
