...
public class RegressionParamsEditorPanel extends JPanel {
    ...
    public RegressionParamsEditorPanel(StandardRegressionParams params, DataModel model) {
        ...
        if (params instanceof LogisticRegressionParams && model instanceof RectangularDataSet) {
            buildMap((RectangularDataSet) model);
    ...
    private static List<Comparable> getSelected(JList list) {
        ...
        List<Comparable> selectedList = new ArrayList<Comparable>(selected == null ? 0 : selected.length);
        if (selected != null) {
    ...
    private void buildMap(RectangularDataSet model) {
