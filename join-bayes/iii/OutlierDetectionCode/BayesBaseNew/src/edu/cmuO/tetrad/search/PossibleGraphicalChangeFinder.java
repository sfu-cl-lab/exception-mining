...
class PossibleGraphicalChangeFinder {
     ...
     */
    public PossibleGraphicalChangeFinder(Pag pag, Collection<Collection<Node>> separations) {
        if(pag == null){
    ...
    private static class DefNonCollider implements GraphicalChange {
		...
		public void apply(Graph g) {
            ...
            Node z = triple.getZ();
            if (g.containsNode(x) && g.containsNode(y) && g.containsNode(z) &&
                    g.isAdjacentTo(x, y) && g.isAdjacentTo(y, z) && !pag.isUnderlineTriple(x, y, z)) {
                pag.addUnderlineTriple(triple);
            }
        }
    }
