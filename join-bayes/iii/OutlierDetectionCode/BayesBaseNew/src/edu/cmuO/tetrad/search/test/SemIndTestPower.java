        ...
        for (int iedges = 0; iedges < edges.length; iedges++) {
            for (int isamp = 0; isamp < samples.length; isamp++) {
                for (int isig = 0; isig < sigs.length; isig++) {
                    ...
                    for (int k = 0; k < 100; k++) {
                        Graph graph = GraphUtils.createRandomDagC(numNodes,
                                numLatents, numEdges);
                        SemPm semPm = new SemPm(graph);

                        SemIm sem = new SemIm(semPm);
                        RectangularDataSet contDataColumnContinuous =
                        ...
                        CovarianceMatrix covarMatrix = new CovarianceMatrix(
                                varNames, covar, sampleSize);
