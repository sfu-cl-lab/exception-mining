...
/**
 * To see what MAG is, look at the graph constraints it uses. </p> Important to
 * inform user that when dynamically adding edges, they may not have a valid MAG
 * until they run closeInducingPaths.  This is done for them when they construct
 * a MAG from another graph.
 *
 ...
 * @see NoEdgeIntoUndirectedEndpoint
 * @see DirectedBidirectedOnly
 */
public final class Mag implements TetradSerializable, Graph {
    ...
    private final static GraphConstraint[] constraints = {
            ...
            new NoEdgeIntoUndirectedEndpoint(), new DirectedBidirectedOnly()};
    ...
    /**
     * Constructs a new MAG from a given graph.  Adds nodes and edges in the
     * order presented by the node and edge iterators.  Nodes or edges which are
     * disallowed are not added.  Inducing paths are closed.
     *
     * @param graph the graph to base the new MAG on.
     * @throws IllegalArgumentException
     */
    public Mag(Graph graph) throws IllegalArgumentException {
        List<GraphConstraint> constraints1 = Arrays.asList(constraints);
     ...
     */
    public static Mag serializableInstance() {
        return new Mag();
    }
