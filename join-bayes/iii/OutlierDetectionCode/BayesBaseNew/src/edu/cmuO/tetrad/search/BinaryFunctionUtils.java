...
/**
 * A gadget to count truth table functions for Clark. Counting the number of
 * truth table functions from a set of "cause" binary variables to an outcome
 * binary variable such that for each variable there is are two rows that differ
 * only in the value of that variable such that the function value for those two
 * rows is different.
 *
...
//                            System.out.println(new BinaryFunction(numArgs, index));
    ...
    public List<BinaryFunction> findNontransitiveTriple() {
        ...
        RandomUtil random = PersistentRandomUtil.getInstance();
