...
/**
 * <p>This class implements the adjacency search step of the Generalize MimBuild
 * algorithm as described on p 362 of CPS, 2nd edition.  The only function that
 * is publically available is adjSearch().</p>
 *
...
public final class MimAdjacencySearch {
    ...
    /**
     * Constructs a new MimAdjacencySearch. IT IS ASSUMED THAT THE INDEPENDENCE
     * CHECKER IS OF CLASS IndTestMimBuild
     *
     * @param graph
     * @param ind
     * @param knowledge
    ...
    /**
     * Return an array of Nodes who can be conditional checks can be done on
     * stipulated by temporal tiers </p> Shane Harwood harwood+@andrew.cmu.edu
     */
    ...
    /**
     * Removes edges from the a graph based on conditional independence facts
     * using conditioning sets of size n and records these conditioning set
     * ("separating sets").  More specifically, for every node x and node y
     * (connected to x), and every set s of size n of nodes connected to x (not
     * including y), checks if x and y are independent conditional on s and, if
     * so, removes the edge from x to y from the graph and records the
     * separating set used to remove that edge.
     *
     ...
     * @param sepset    As edges are removed, their separating sets (the sets
     *                  conditional on which their endpoints are independent)
     *                  are recorded in this sepset.  This is for use later on
     *                  in the algorithm.
     * @return false if no independencies are found for this step.  (This means
     *         we are finished with adjacency search.)
     */
    ...
    /**
     * The ChoiceGenerator supplies an int array.  This class will transform
     * that int array (interpretted as indices array of objects) into an  into a
     * List of objects
     *
