...
/**
 * Implementation of a test of tetrad constraints with discrete variables. We
 * are assuming that variables are ordinal or binary. Such tests are a core
 * statistical procedure in algorithms BuildPureClusters and Purify. </p> An
 * "underlying latent variable" approach is used to test tetrads indirectly by
 * fitting discrete one-factor and two-factor models. See Bartholomew and Knott
 * (1999) for details. A two-stage procedure for fitting polychorics
 * correlations (Olsson, 1979) and a chi-square test of tetrad constraints over
 * those correlations is the key for this method. </p> References: </p>
 * Bartholomew, D. and Knott, M. (1999). Latent Variable Models and Factor
 * Analysis, 2nd edition. Arnold. </p> Olsson, Ulf (1979). "Maximum likelihood
 * estimation of the polychoric correlation coefficient". Psychometrika 44,
 * 443-460. </p> Stroud, A. and Secrest D. (1966) Gaussian Quadrature Formulas.
 * Prentice Hall.
 *
 * @author Ricardo Silva
 * @version $Revision: 6124 $ $Date: 2006-01-12 08:34:58 -0500 (Thu, 12 Jan
 *          2006) $
 */
...
//    int rawdata[][];
    ...
    public DiscreteTetradTest(RectangularDataSet dataSet, double sig) {
...
//        computeCounts(this.counts, this.rawdata);
	...
	public int tetradScore(int i, int j, int k, int l) {
        if (oneFactorTest(i, j, k, l)) {
            ...
            if (this.prob[2] < this.prob[0] && this.prob[2] < this.prob[0]) {
                this.tempProb = this.prob[0];
    ...
    /**
     * Tests the tetrad (v1, v3) x (v2, v4) = (v1, v4) x (v2, v3), and only
     * that.
     */
...
//    private void computeCounts(int counts[][][][], int data[][]) {
//        int numRows = this.dataSet.getNumRows();
//        int numColumns = this.dataSet.getNumColumns();
//        for (int i = 0; i < numColumns; i++) {
...
//            for (int i = 0; i < numColumns; i++) {
//                for (int j = i; j < numColumns; j++) {
//                    counts[i][j][getValuePosition(data[i][r], i)][getValuePosition(
//                            data[j][r], j)]++;
//                }
//            }
//        }
...
//    }
    ...
    private void computeCounts(int counts[][][][], RectangularDataSet data) {
        int numRows = this.dataSet.getNumRows();
        int numColumns = this.dataSet.getNumColumns();
        for (int i = 0; i < numColumns; i++) {
            for (int j = i; j < numColumns; j++) {
...
//                                                    this.rawdata[y][r], y)]++;
    ...
    /**
     * Returns the position of a specific value in a natural (0, 1, 2, ...)
     * scale for a given variable.
     */
    ...
    private double[][] getUnderlyingCorr(int nextCounts[][][][]) {
        ...
        for (int i = 0; i < outputCorr.length; i++) {
            dummyNames[i] = "L" + i;
            for (int j = 0; j <= i; j++) {
                System.out.print(((int) (100. * outputCorr[i][j])) /
                        100. + "\t");
            }
            System.out.println();
        }
    ...
    /**
     * Function to be minimized: -loglikelihood. It *is* assumed that
     * computeFiBuffer and computeCurrentPi were called before this method.
     */
    ...
    /**
     * Optimizing the polychoric correlation by searching over a uniform grid in
     * (-1, 1).
     */
	...
	public boolean oneFactorTest(int i, int j, int k, int l) {
        ...
        CovarianceMatrix covarianceMatrix =
                new CovarianceMatrix(vNames, ulCorr, this.dataSet.getNumRows());
        ...
        CovarianceMatrix covMatrix = new CovarianceMatrix(vNames,
                est.getEstimatedSem().getImplCovarMeas().toArray(),
        ...
        /*System.out.println("(" + i + " " + j + " " + k + " " + l + " chisq = " +
                  est.getEstimatedSem().getChiSquare() + ", prob = " + est.getEstimatedSem().getPValue() + ")");
        System.out.println(1. - ProbUtils.chisqCdf(est.getEstimatedSem().getChiSquare(), est.getEstimatedSem().getDof() + 1));
        return est.getEstimatedSem().getPValue() > this.sig;*/
    }
	...
	public boolean oneFactorTest(int i, int j, int k, int l, int x) {
        ...
        CovarianceMatrix CovarianceMatrix =
                new CovarianceMatrix(vNames, ulCorr, dataSet.getNumRows());
        Graph factorModel = new EdgeListGraph();
        Node eta = new GraphNode("eta");
        eta.setNodeType(NodeType.LATENT);
        factorModel.addNode(eta);
    ...
    private double piHat2(int indices[], int v[], SemIm semIm) {
        ...
        Node errorEta2;
        if (semIm.getSemPm().getGraph().getParents(etaNode2).get(0) == etaNode1)
        {
            errorEta2 = semIm.getSemPm().getGraph().getParents(etaNode2)
                    .get(1);
        }
        else {
            errorEta2 = semIm.getSemPm().getGraph().getParents(etaNode2)
