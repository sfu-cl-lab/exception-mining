...
/**
 * Purify is a implementation of the automated purification methods described on
 * CPS and the report "Learning Measurement Models" CMU-CALD-03-100.
 * <p/>
 ...
 * <p/>
 * Drton, M. and Richardson, T. (2003). "Iterative conditional fitting for
 * Gaussian ancestral graphical models".  Technical report, Department of
 ...
 * @version $Revision: 6481 $ $Date: 2006-01-12 08:34:58 -0500 (Thu, 12 Jan
 *          2006) $
 */
...
public class Purify {
    ...
    /**
     * Information for the search method
     */
    public static final int TEST_POPULATION = -1;

    public static final int TEST_GAUSSIAN_PVALUE = 0;
    public static final int TEST_TETRAD_BOLLEN = 1;
    public static final int TEST_TETRAD_WISHART = 2;
    public static final int TEST_GAUSSIAN_SCORE_MARKS = 3;
    public static final int TEST_GAUSSIAN_SCORE = 4;
    public static final int TEST_GAUSSIAN_SCORE_ITERATE = 5;
    public static final int TEST_NONE = -2;

    public static final int TEST_DISCRETE_LRT = 1;
    public static final int TEST_DISCRETE_VARIATIONAL = 2;
    ...
    public Purify(CorrelationMatrix correlationMatrix, double sig, int testType,
            Clusters clusters) {
        ...
        if (testType == TEST_TETRAD_BOLLEN) {
    ...
    public Purify(RectangularDataSet dataSet, double sig, int testType,
            Clusters clusters) {
    ...
    private void initAlgorithm(double sig, int testType, Clusters clusters) {
        ...
        if (this.tetradTest == null) {
            if (correlationMatrix != null) {
                int type = -1;
                switch (testType) {
                    case TEST_GAUSSIAN_SCORE:
                        type = ContinuousTetradTest.TEST_GAUSSIAN_SCORE;
                        break;
                    case TEST_GAUSSIAN_SCORE_MARKS:
                        type = ContinuousTetradTest.TEST_GAUSSIAN_SCORE_MARKS;
                        break;
                    case TEST_GAUSSIAN_SCORE_ITERATE:
                        type = ContinuousTetradTest.TEST_GAUSSIAN_SCORE_ITERATE;
                        break;
                    ...
                    case TEST_TETRAD_BOLLEN:
                        ...
                        break;
                    case TEST_GAUSSIAN_PVALUE:
                        type = ContinuousTetradTest.TEST_GAUSSIAN_PVALUE;
                        break;
                    case TEST_NONE:
                        type = ContinuousTetradTest.TEST_NONE;
                        break;
                    default:
                        assert false;
                }
        ...
        numV = tetradTest.getVarNames().length;
        outputMessage = true;
    }

    public static String[] getTestDescriptions() {
        String testNames[] = new String[5];
        testNames[0] = "Gaussian BIC";
        testNames[1] = "Gaussian BIC (mark only)";
        testNames[2] = "Wishart's Tetrad Test";
        testNames[3] = "Bollen's Tetrad Test";
        testNames[4] = "Gaussian P-value";
        return testNames;
    }
    ...
    private Graph convertSearchGraph(List clusters) {
        ...
        for (int i = 0; i < clusters.size(); i++) {
            ...
            for (int j = 0; j < indicators.length; j++) {
                ...
                indicatorName = tetradTest.getVarNames()[indicators[j]];
        ...
        for (int i = 0; i < clusters.size(); i++) {
            ...
            for (int j = i + 1; j < clusters.size(); j++) {
                graph.setEndpoint((Node) nodes.get(i), (Node) nodes.get(j),
                        Endpoint.ARROW); //TODO: put CIRCLES back
                graph.setEndpoint((Node) nodes.get(j), (Node) nodes.get(i),
                        Endpoint.TAIL);
            }
        }
    ...
    private Graph convertSearchGraph(SemGraph input) {
        ...
        Graph output = new EndpointMatrixGraph(allNodes);
    ...
    /**
     * ---------------------------------------------------------------------------------------------------------------
     * TETRAD-BASED PURIFY </p> - using tetrad constraints - This method checks
     * if there is any evidence that a node is impure. If there is not, then it
     * is treated as pure. This is virtually the original Purify as described in
     * CPS.
     */

    private List tetradBasedPurify(List partition) {
        boolean eliminated[] = new boolean[numV];
        ...
        for (Iterator it = partition.iterator(); it.hasNext();) {
            intraConstructPhase((int[]) it.next(), eliminated, "T" + (++count));
        }
        printlnMessage();
        ...
        printlnMessage("----------------------");
        printlnMessage();
        crossConstructPhase(partition, eliminated);
        printlnMessage();
    ...
    /**
     * Marks for deletion nodes within a single cluster that are part of some
     * tetrad constraint that does not hold according to a statistical test.
     * </p> False discovery rates will be used to adjust for multiple hypothesis
     * tests.
     *
     * @param cluster
     * @param eliminated
    ...
    private void intraConstructPhase(int cluster[], boolean eliminated[],
        ...
        int totalpvalues = numRemaining * (numRemaining - 1) *
                (numRemaining - 2) * (numRemaining - 3) * 3;
        if (totalpvalues == 0) {
            return;
        }
        double allPValues[] = new double[totalpvalues];
        int numImpurities = 0;
            ...
            for (int j = i + 1; j < clusterSize - 2; j++) {
                ...
                for (int k = j + 1; k < clusterSize - 1; k++) {
                    ...
                    for (int l = k + 1; l < clusterSize; l++) {
                        ...
                        pvalues[i][j][k][l][0] = allPValues[count++] =
                                tetradTest.tetradPValue(cluster[i], cluster[j],
                                        cluster[k], cluster[l]);
                        pvalues[i][j][k][l][1] = allPValues[count++] =
                                tetradTest.tetradPValue(cluster[i], cluster[j],
                                        cluster[l], cluster[k]);
                        pvalues[i][j][k][l][2] = allPValues[count++] =
                                tetradTest.tetradPValue(cluster[i], cluster[k],
                                        cluster[l], cluster[j]);
                    }
                }
            }
        ...
        if (count > 1) {
            Arrays.sort(allPValues, 0, count - 1);
        }
        //for (int i = 0; i < count; i++)
        //    System.out.println(allPValues[i]);
        System.out.println("numRemaining = " + numRemaining);
        System.out.println("allPValues = " + allPValues.length);
        int c = 0;
        while (allPValues[c] <
                tetradTest.getSignificance() * (c + 1.) / totalpvalues &&
                c < count) {
            c++;
        }
        double cutoff = allPValues[c];
        System.out.println("cutoff = " + allPValues[c]);
        for (int i = 0; i < clusterSize - 3; i++) {
            if (eliminated[cluster[i]]) {
                continue;
            }
        ...
        while (numImpurities > 0) {
            ...
            totalpvalues = numRemaining * (numRemaining - 1) *
                    (numRemaining - 2) * (numRemaining - 3) * 3;
            printlnMessage("Dropped " +
                    tetradTest.getVarNames()[cluster[max_index]] +
                    "  Without it, " + numImpurities + " of " + totalpvalues +
                    " fail the FDR test.");
        }
    }

    /**
     * Marks for deletion nodes that are part of some tetrad constraint between
     * two clusters that does not hold according to a statistical test. </p>
     * False discovery rates will be used to adjust for multiple hypothesis
     * tests.
     *
     * @param partition
     * @param eliminated
    ...
    private void crossConstructPhase(List partition, boolean eliminated[]) {
        ...
        for (int p1 = 0; p1 < partition.size(); p1++) {
            int cluster1[] = (int[]) partition.get(p1);
            int n1 = 0;
            ...
            for (int p2 = p1 + 1; p2 < partition.size(); p2++) {
                int cluster2[] = (int[]) partition.get(p2);
                int n2 = 0;
                for (int i = 0; i < cluster2.length; i++) {
                    if (!eliminated[cluster2[i]]) {
                        n2++;
                    }
                }
                if (n1 >= 3) {
                    totalpvalues += n1 * (n1 - 1) * (n1 - 2) * n2;
                }
                if (n2 >= 3) {
                    totalpvalues += n2 * (n2 - 1) * (n2 - 2) * n1;
                }
                if (n1 >= 2 && n2 >= 2) {
                    totalpvalues += n1 * (n1 - 1) * n2 * (n2 - 2);
                }
            }
        }
        ...
        for (int i = 0; i < partition.size(); i++) {
            int cluster[] = (int[]) partition.get(i);
        ...
        for (int p1 = 0; p1 < partition.size(); p1++) {
            int cluster1[] = (int[]) partition.get(p1);
            for (int p2 = 0; p2 < partition.size(); p2++) {
                if (p1 == p2) {
                    continue;
                }
                int cluster2[] = (int[]) partition.get(p2);
                    ...
                    for (int j = i + 1; j < cluster1.length - 1; j++) {
                        ...
                        for (int k = j + 1; k < cluster1.length; k++) {
                            ...
                            for (int l = 0; l < cluster2.length; l++) {
                                ...
                                allPValues[count++] = tetradTest.tetradPValue(
        ...
        if (numImpurities > 0) {
            printlnMessage("Iteration 1   " + numImpurities + " of " +
                    totalpvalues + " tetrads fail the FDR test.");
        }
        else {
        ...
        while (numImpurities > 0) {
            ...
            for (int p = 0; p < partition.size(); p++) {
                int cluster[] = (int[]) partition.get(p);
            ...
            eliminated[((int[]) partition.get(max_index_p))[max_index_i]] =
                    true;
            ...
            for (int p = 0; p < partition.size(); p++) {
                ...
                for (int i = 0; i < cluster.length; i++) {
                    ...
                    for (Iterator it = failures[p][i].iterator(); it.hasNext();)
                    {
                        int impurity[] = (int[]) it.next();
                        for (int j = 0; j < 4; j++) {
                            if (impurity[j] == ((int[]) partition.get(
            ...
            printlnMessage("Dropped " +
                    tetradTest.getVarNames()[(((int[]) partition.get(
                            max_index_p))[max_index_i])] + "  Without it, " +
                    numImpurities + " of " + totalpvalues +
                    " tetrads fail the FDR test.");
        }
    }
    ...
    /**
     * ---------------------------------------------------------------------------------------------------------------
     * TETRAD-BASED PURIFY2 </p> - using tetrad constraints - Second variation:
     * this method checks for each pair (X, Y) if there is another pair (W, Z)
     * such that all three tetrads hold in (X, Y, W, Z). If not, this pair is
     * marked as impure. This is more likely to leave true impurities in the
     * estimated graph, but on the other hand it tends to remove less true pure
     * indicators. We also do not use all variables as the domain of (W, Z):
     * only those in the given partition, in order to reduce the number of false
     * positives. In my opinion, tetradBasedPurify2 is a better compromise thatn
     * tetradBasedPurify1, but one might want to test it with a larger variety
     * of graphical structures and sample size. </p> -- Ricardo Silva
     */
    ...
    /**
     * Verify if a pair of indicators is impure, or if there is no evidence they
     * are pure.
     */
    private boolean[][] tetradBasedMarkImpurities(List clustering) {
        printlnMessage("   (searching for impurities....)");
        int relations[][] = new int[numV][numV];
        for (int i = 0; i < numV; i++) {
        ...
        boolean impurities[][] = new boolean[numV][numV];
        for (int i = 0; i < numV; i++) {
            for (int j = 0; j < numV; j++) {
...
//     ones.
    ...
    /**
     * This initialization has to be done only once per complete search. No need
     * to call it multiple times for each stage of purifyScoreSearch
     */
    ...
    private void structuralEmInitialization(List partition) {
        ...
        //        this.purePartitionGraph = new SemGraph(new ProtoSemGraph(basicGraph));
    ...
    /**
     * Estimate parameters of a general measurement model with possible
     * impurities but where each indicator may have multiple latent and observed
     * parents.
     */
    ...
    /**
     * The expectation step for the structural EM algorithm. This is heavily
     * based on "EM Algorithms for ML Factor Analysis", by Rubin and Thayer
     * (Psychometrika, 1982)
     */
...
//                Iterator ci = semIm.getSemPm().getGraph().getChildren(exo1)
//                        .iterator();
//                exo1 =
//                        (Node) ci.next(); //Assuming error nodes have only one children in SemGraphs...
//                ci = semIm.getSemPm().getGraph().getChildren(exo2).iterator();
//                exo2 =
//                        (Node) ci.next(); //Assuming error nodes have only one children in SemGraphs...
    ...
    private double scoreCandidate() {
        ...
        return -semIm.getNegTruncLL() - 0.5 * semIm.getNumFreeParams() *
    ...
    /**
     * Find the MLE of a latent SemPm with double directed-edges using Drton and
     * Richardson (2003). It is assumed that data matrices such as betas,
     * latentBetas, covErrors and covLatentErrors have been allocated to memory,
     * and indexing matrices such as parent as spouses allocated and
     * initialized. Such initialization is usually done inside the gaussianEM
     * method.
     */
    ...
    private double gaussianMaximization(SemIm semIm) {
        ...
        //semIm = SemIm.newInstance(realIm.getSemPm());
                ...
                for (int ii = 0; ii < this.nSpouses[i]; ii++) {
                    for (int j = ii; j < this.nSpouses[i]; j++) {
                        ...
                        if (this.pseudoParentsCov[i][this.parents[i].length +
                            ...
                            System.out.println(semIm.getSemPm().getDag());
...
//                for (Node parent : semIm.getSemPm().getGraph().getParents(node)) {
...
//                        for (Iterator it = semIm.getSemPm().getGraph()
...
//                        }
    ...
    private List findInducedPureGraph(List partition, boolean impurities[][]) {
        ...
        for (int i = 0; i < elements.length; i++) {
            eliminated[elements[i][0]] =
                    impurities[elements[i][0]][elements[i][0]];
        }
        while (!validSolution(elements, eliminated)) {
            ...
            //for (int i = 0; i < elements.length; i++)
            //  if (elements[i][2] > 0)
            //        printlnMessage("#@@@@ " + this.measuredNodes.get(elements[i][0]).toString());
