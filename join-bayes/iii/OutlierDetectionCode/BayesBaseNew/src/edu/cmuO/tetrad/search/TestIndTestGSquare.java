...
public class TestIndTestGSquare extends TestCase {
    ...
    public void testIsIndependent() {
        try {
            RectangularDataSet dataSet = getDataSet(1);
...
//            z.add(dataSet.getVariable("X2"));
    ...
    public RectangularDataSet getDataSet(int i) throws IOException {
        String filename = discreteFiles[i];
        System.out.println("Loading " + filename);

        DataParser parser = new DataParser();
        parser.setMissingValueMarker("-99");
        return parser.parseTabular(new File(filename));
    }
