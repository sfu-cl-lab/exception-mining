...
//        out.println(prototypes.viewDice());
    ...
    public static void initRandomly(DoubleMatrix2D x) {
        for (int i = 0; i < x.rows(); i++) {
            for (int j = 0; j < x.columns(); j++) {
                x.set(i, j, PersistentRandomUtil.getInstance().nextDouble());
            }
        }
    }
    ...
    static DoubleMatrix2D restrictToTimeSeries(DoubleMatrix2D data) {
        return data.viewPart(0, 3, data.rows(), data.columns() - 3);
    }

    static DoubleMatrix2D restrictToXyz(DoubleMatrix2D data) {
