...
public class Shimizu2006SearchOld implements SemLearningMethod {
	...
	Shimizu2006SearchOld(double alpha){
		this.alpha = alpha;
	}
 ...
 */
	public static ColtDataSet lingamDiscovery_Bhat(RectangularDataSet dataSet) {
...
//		System.out.println("containsAllZeros: returning true");
	...
	public GraphWithParameters lingamDiscovery_DAG(RectangularDataSet dataSet) {
 ...
 */
	public static GraphWithParameters lingamDiscovery_DAG(RectangularDataSet dataSet, double alpha) {
...
//		System.out.println("after pruning, B = " + B);
	...
	private static void pruneEdges(ColtDataSet B, RectangularDataSet dataSet, double alpha, int nSamples) {
	 ...
	 */
	private static void pruneEdgesBySampling(ColtDataSet B, RectangularDataSet dataSet,
		...
		for (int i=0; i<nSamples; i++){
			...
			while(bHat==null){
				RectangularDataSet currentBootstrapSample = isBootstrapSampling ? getBootstrapSample(dataSet) : //randomly draw from the dataSet, with replacement
...
//					System.out.println("not pruning: outside interval");
	 ...
	 */
	private static int diversity(RectangularDataSet currentDataSet) {
		int n=0;
 ...
 */
	 ...
	 */
	private static RectangularDataSet getSubsetOfDataSet(RectangularDataSet dataSet, int pieces, int pieceIndex) {
		DoubleMatrix2D mat = dataSet.getDoubleData();
	...
	private static RectangularDataSet getBootstrapSample(RectangularDataSet dataSet) {
		RectangularDataSet sample = null;
		int diversity = 0;
...
//			System.out.println("diversity = " + diversity);
	 ...
	 */
	private static RectangularDataSet makeBootstrapSample(RectangularDataSet dataSet) {
		DoubleMatrix2D mat = dataSet.getDoubleData();
...
//		System.out.println("bootstrapSample = " + bootstrapSample);
	 ...
	 *
	 */
	private static RectangularDataSet normalizeVariance(RectangularDataSet dataSet) {
		DoubleMatrix2D mat = dataSet.getDoubleData();
...
//	}
	...
	@Override
	public GraphWithParameters run(RectangularDataSet dataSet, boolean estimateCoefficients, TestFastIca.PwpPlusGeneratingParameters standardPwpPlusParms) {
		return run(dataSet);
	}

    /**
     * make up the list of nodes and call the other makeDataSet() with it
     *
     * @param inVectors
     * @return
     */
    public static ColtDataSet makeDataSet(DoubleMatrix2D inVectors) {
        ...
        return makeDataSet(inVectors, nodes);
    }

    //Gustavo 14 May 2007
    //this creates a list of ContinuousVariables from 'nodes'
