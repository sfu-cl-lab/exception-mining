...
/**
 * Implements ICF as specified in Drton and Richardson (2003), Iterative
 * Conditional Fitting for Gaussian Ancestral Graph Models, using hints from
 * previous implementations by Drton in the ggm package in R and by Silva in the
 * Purify class. The reason for reimplementing in this case is to take advantage
 * of linear algebra optimizations in the COLT library.
 *
...
public class Ricf {
    ...
    public RicfResult ricf(Graph mag, CovarianceMatrix covMatrix, int sampleSize,
            double tolerance) {
        DoubleFactory2D factory = DoubleFactory2D.dense;
        Algebra algebra = new Algebra();

        DoubleMatrix2D S = covMatrix.getMatrix();
        ...
        if (ug.length > 0) {
            ...
            CovarianceMatrix ugCov = covMatrix.getSubmatrix(ug);
        ...
        return new RicfResult(sigmahat, lambdahat, bhat, omegahat, i);
    }

    /**
     * Returns an enumeration of the cliques of the given graph considered as
     * undirected.
     */
     ...
     */
    public FitConGraphResult fitConGraph(Graph graph, CovarianceMatrix cov, int n, double tol) {
        DoubleFactory2D factory = DoubleFactory2D.dense;
        Algebra algebra = new Algebra();
    ...
    /**
     * If L2 is nonempty, moves nodes from L2 to L1 that can be added to L1.
     * Nodes less than max(L1) are not considered--i.e. L1 is being extended
     * to the right. Nodes not greater than the most recently moved node are
     * not considered--this is a mechanism for
     */
    ...
    public static class RicfResult {
        ...
        public RicfResult(DoubleMatrix2D shat, DoubleMatrix2D lhat, DoubleMatrix2D bhat,
                DoubleMatrix2D ohat, int iterations) {
            this.shat = shat;
            this.lhat = lhat;
            this.bhat = bhat;
            this.ohat = ohat;
