...
public class Lingam {
     ...
     */
    public GraphWithParameters lingam(RectangularDataSet dataSet) {
...
//                else {
//                    BTilde.getData().set(xi, xj, meanMat.get(i, j));
//                }
            }
        }
    }
    ...
    private void pruneEdgesByRegression(Data BTilde, Data data, double alpha) {
        ...
        RectangularDataSet _data = ColtDataSet.makeContinuousData(variables, data.getData());
    ...
    private void pruneEdgesByRegression2(Data BTilde, Data data, double alpha) {
        ...
        for (Node y : graph.getNodes()) {
            ...
            for (int j = 0; j < numSamples; j++) {
                ...
                RectangularDataSet _data = ColtDataSet.makeContinuousData(variables, bootstrap.getData());
                Regression regression = new RegressionDataset(_data);
...
//                    BTilde.getData().set(i, j, 0);
//                }
//                else {
//                    BTilde.getData().set(i, j, themean);
//                }
