...
/**
 * An adaptation of GES to be used as a global reorientation procedure.
 * Original code by Ricardo Silva, cleaned up by Joe Ramsey. Only a few methods
 * and constructors have changed from that version.
 *
...
public final class GesOrienter implements Reorienter {
    ...
    /**
     * Source graph, used as a source of possible adjacencies for edge
     * orientation.
     */
    ...
    public GesOrienter(RectangularDataSet dataSet, Knowledge knowledge) {
        setDataSet(dataSet);
    ...
    public GesOrienter(CovarianceMatrix covMatrix) {
        setCorrMatrix(new CorrelationMatrix(covMatrix));
        initialize(10., 1.);  // structure prior was 0.001.
    }

    public GesOrienter(RectangularDataSet dataSet, Graph trueGraph) {
        setDataSet(dataSet);
        initialize(10., 1.);
...
//            this.variances = dataSet.getCorrelationMatrix();
    ...
    private static void print(String message) {
        TetradLogger.getInstance().details(message);
    }
