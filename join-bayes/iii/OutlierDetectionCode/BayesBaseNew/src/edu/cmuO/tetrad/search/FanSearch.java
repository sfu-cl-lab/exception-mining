 ...
 * @version $Revision: 4874 $ $Date: 2006-01-20 13:34:55 -0500 (Fri, 20 Jan
 *          2006) $
 */
public final class FanSearch {
    ...
    /**
     * The point at which the algorithm will stop. Must be one of
     * UNTRIMMED_UNORIENTED_ADJACENCY_GRAPH, UNTRIMMED_ORIENTED_ADJACENCY_GRAPH,
     * or TRIMMED_ORIENTED_ADJACENCY_GRAPH.
     */
    private int stopPoint = FanSearch.TRIMMED_ORIENTED_ADJACENCY_GRAPH;
     ...
     */
    public FanSearch(IndependenceTest test, int depth) {
        if (test == null) {
     ...
     */
    public Graph search(List<String> targetNames) {
...
//        }
        ...
        for (String name : targetNames) {
            targets.add(getVariableForName(name));
        }
        ...
        TetradLogger.getInstance().info("target = " + getTargets());
        ...
        for (Node node : visited) {
            adjacents.addAll(graph.getAdjacentNodes(node));
        }

        adjacents.removeAll(visited);

        for (Node v : adjacents) {
...
//        LogUtils.getInstance().fine("After step 3 (prune PCPC)" + graph);
        ...
        FanSearch.orientCollidersLocally(bk, graph, getTest(), getDepth(), _visited);
        FanSearch.orientUsingMeekRulesLocally(bk, graph, getTest(), getDepth(), _visited);

        if (getStopPoint() == FanSearch.UNTRIMMED_ORIENTED_ADJACENCY_GRAPH) {
            finishUp(start, graph);
            return graph;
        }
...
//        List<Node> trimNodes = new ArrayList<Node>();
//        trimNodes.add(this.targets);
//        trimNodes.addAll(graph.getAdjacentNodes(this.targets));

//        MbUtils.trimToNeighborhood(graph, trimNodes);
...
//     */
//    public List<Graph> generateMbDags(boolean orientBidirectedEdges) {
//        return new LinkedList<Graph>(listDags(new EdgeListGraph(resultGraph()),
//                orientBidirectedEdges));
//    }
//
//    public List<Graph> generateMbDags(Graph resultGraph,
...
//    }
    ...
    public void setStopPoint(int stopPoint) {
        switch (stopPoint) {
            case FanSearch.UNTRIMMED_UNORIENTED_ADJACENCY_GRAPH:
            case FanSearch.UNTRIMMED_ORIENTED_ADJACENCY_GRAPH:
            case FanSearch.TRIMMED_ORIENTED_ADJACENCY_GRAPH:
                break;
            default:
...
//        List<Node> nodes = graph.getNodes();
    ...
    public static void orientUsingMeekRulesLocally(Knowledge knowledge,
        ...
        while (true) {
            if (FanSearch.meekR1Locally(graph, knowledge, test, depth, neighborhood)) {
                continue;
            }

            if (FanSearch.meekR2(graph, knowledge, neighborhood)) {
                continue;
            }

            if (FanSearch.meekR3(graph, knowledge, neighborhood)) {
                continue;
            }

            if (FanSearch.meekR4(graph, knowledge, neighborhood)) {
                continue;
            }
            ...
            for (Node a : neighborhood) {
                ...
                while ((combination = cg.next()) != null) {
                    ...
                    if (graph.getEndpoint(b, a) == Endpoint.ARROW &&
                        ...
                        if (FanSearch.isArrowpointAllowed(a, c, knowledge)) {
                        ...
                        if (FanSearch.isArrowpointAllowed(a, b, knowledge)) {
        ...
        for (Node a : neighborhood) {
            ...
            while ((combination = cg.next()) != null) {
                ...
                if (graph.isDirectedFromTo(b, a) &&
                    ...
                    if (FanSearch.isArrowpointAllowed(b, c, knowledge)) {
                    ...
                    if (FanSearch.isArrowpointAllowed(c, b, knowledge)) {
...
//                    }
                    ...
                    if (graph.isDirectedFromTo(b, c) &&
                        ...
                        if (FanSearch.isArrowpointAllowed(a, d, knowledge)) {
