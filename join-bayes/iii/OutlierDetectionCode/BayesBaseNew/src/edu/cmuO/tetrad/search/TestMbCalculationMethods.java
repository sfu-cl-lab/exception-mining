...
/**
 * Compares MBF to Regression on the task of calculating a Markov blanket from
 * continuous data.
 *
 * @author Joseph Ramsey
 * @version $Revision: 6083 $ $Date: 2006-01-10 15:52:32 -0500 (Tue, 10 Jan
 *          2006) $
 */
...
//        System.out.println(" FP(MB)\t FN(MB)\t Truth");
        ...
        do {
            Dag randomGraph = GraphUtils.createRandomDag(numVars, 0, numEdges,
                    40, 40, 40, false);
            ...
            RectangularDataSet dataSet = semIm1.simulateData(sampleSize, latentDataSaved);
            ...
            MbFanSearch mbSearch = new MbFanSearch(test, depth);
...
//                }
    ...
    /**
     * MBF for all variables in a data set, with markups on output to indicate
     * FN, etc.
     */
    public void rtest4() {
        NumberFormat nf = NumberFormatUtil.getInstance().getNumberFormat();
...
//        RectangularDataSet dataSet = semIm1.simulateData(sampleSize);
        RectangularDataSet dataSet = simulator.simulateDataAcyclic(sampleSize);
...
//        IndependenceTest test = new IndTestChiSquare(dataSet, alphaMbf);
    ...
    /**
     * Looks at the effect of using GES as a post-processing filter to reduce
     * false positives coming out of MBF.
     */
    public void rtest7() {
        double alphaMbf = 0.05;
        int numVars = 1000;
        ...
        System.out.println("... creating random DAG");
        Dag randomGraph = GraphUtils.createRandomDag(numVars, 0, numEdges, 4,
                3, 3, false);
...
//        IndependenceTest test = new IndTestFisherZD(dataSet, alphaMbf);
        ...
        for (int j = 0; j < 50; j++) {
            int i = PersistentRandomUtil.getInstance().nextInt(
                    graphNodes.size());
