...
public final class MlBayesIm implements BayesIm {
    ...
    private static final RandomUtil randomUtil =
            PersistentRandomUtil.getInstance();
//    RandomUtil randomUtil = new SeededRandomUtil(522343L);
	...
	public RectangularDataSet simulateData(int sampleSize, boolean latentDataSaved) {
        return simulateDataHelper(sampleSize, randomUtil, latentDataSaved);
    }

    @Override
	public RectangularDataSet simulateData(RectangularDataSet dataSet, boolean latentDataSaved) {
        return simulateDataHelper(dataSet, randomUtil, latentDataSaved);
    }
     ...
     * @return the simulated sample as a DataSet.
     */
    public RectangularDataSet simulateData(int sampleSize, long seed, boolean latentDataSaved) {
        return simulateDataHelper(sampleSize, new SeededRandomUtil(seed), latentDataSaved);
    }

    public RectangularDataSet simulateData(RectangularDataSet dataSet, long seed, boolean latentDataSaved) {
        return simulateDataHelper(dataSet, new SeededRandomUtil(seed), latentDataSaved);
    }
    ...
    private void initializeNode(int nodeIndex, BayesIm oldBayesIm,
        ...
        for (int dim : dims) {
            if (numRows > Integer.MAX_VALUE / dim) {
                throw new IllegalArgumentException(
                        "The number of rows in the " +
                                "conditional probability table for " +
                                nodes[nodeIndex] +
                                " is greater than Integer.MAX_INT and cannot be " +
                                "represented.");
            }
    ...
    private static double[] getRandomWeights(int size) {
        ...
        int randomCell = PersistentRandomUtil.getInstance().nextInt(size);
