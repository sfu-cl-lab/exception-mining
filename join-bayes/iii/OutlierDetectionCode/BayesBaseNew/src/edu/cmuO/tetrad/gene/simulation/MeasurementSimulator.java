 ...
 * @version $Revision: 5308 $ $Date: 2005-07-07 17:17:27 -0400 (Thu, 07 Jul
 *          2005) $
...
public class MeasurementSimulator implements TetradSerializable {
     ...
     * needed for the current cell being simulated. It's usually a good idea to
    ...
    /**
     * Returns true if raw data is being saved in the current simulation, false
     * if not.
     *
     * @see #setRawDataSaved
     */
    ...
    /**
     * Returns 'true' if measured data is being saved out for the current
     ...
     * @see edu.cmu.tetrad.gene.history.GeneHistory
     */
     ...
     *
     * @see edu.cmu.tetrad.gene.history.DishModel
     */
     ...
     * degradation in quality of measurement with each reuse. In the current
    ...
    private void constructTimeSteps() {
        ...
        int numTimeSteps = (1 + stepsGenerated - firstStepStored) / interval;

        this.timeSteps = new int[numTimeSteps];

        for (int i = 0; i < timeSteps.length; i++) {
            this.timeSteps[i] = firstStepStored + i * interval;
        }
    }
     ...
     * (This is not necessary; it's just how the current parameters
    ...
    public void simulate(GeneHistory history) {
        ...
        DishModel dishModel =
                new DishModel(numDishes, getDishDishVariability());
        ...
        if (measuredDataSaved) {
            int numSteps = timeSteps.length;
            int numSamples = numDishes * numChips;
        ...
        if (isRawDataSaved()) {
            int numSteps = timeSteps.length;
            int numCells = numDishes * getNumCellsPerDish();
        ...
        Distribution sampleErrorDist = new NormalDistribution(0.0, sampleSd);
        Distribution chipErrorDist = new NormalDistribution(0.0, chipSd);
        Distribution pixelErrorDist = new NormalDistribution(0.0, pixelSd);
        double[][] chipErrors = new double[numDishes][numChips];
        double[][] sampleErrors = new double[numDishes][numChips];
        ...
        for (int d = 0; d < numDishes; d++) {
            ...
            for (int c = 0; c < numCellsPerDish; c++) {
                ...
                while (++s < stepsGenerated) {
                    ...
                    if (s == timeSteps[sIndex] - 1) {
                        ...
                        for (int f = 0; f < numFactors; f++) {
                            ...
                            // Copy data from the current time step in
            ...
            if (measuredDataSaved) {
                ...
                for (int sIndex = 0; sIndex < timeSteps.length; sIndex++) {
                    for (int f = 0; f < numFactors; f++) {
                        for (int ch = 0; ch < numChips; ch++) {
                            double average =
                                    aggregation[sIndex][f] / numCellsPerDish;
    ...
    /*
        ...
        setStepsGenerated(), setInterval, and setFirstStepStored() methods
