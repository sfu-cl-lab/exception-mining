 ...
 * @version $Revision: 6563 $ $Date: 2006-01-09 11:04:56 -0500 (Mon, 09 Jan
 *          2006) $
 */
public final class TestDiscretizer extends TestCase {
    ...
    public static void testBreakpointCalculation(){
        ...
        double[] breakpoints = Discretization.getEqualFrequencyBreakPoints(data, 3);

        assertTrue(breakpoints.length == 2);
        assertEquals(4.5, breakpoints[0]);
        assertEquals(10.1, breakpoints[1]);

        Discretization dis = Discretization.discretize(data, breakpoints, "after", Arrays.asList("1", "2", "3"));
        System.out.println(dis);

        breakpoints = Discretization.getEqualFrequencyBreakPoints(data, 4);
    ...
    public static void testContinuous() {
        ...
        Discretization discretization = Discretization.discretize(data, cutoffs, "after", categories);
