 ...
 * @see BayesIm
 */
public final class BayesPm implements VariableSource, TetradSerializable {
	 ...
	 * values named "value1" and "value2".
	 */
	public BayesPm(Dag graph) {
		this(graph, 2, 2);
	}
	 ...
	 */
	public BayesPm(Dag graph, BayesPm oldBayesPm) {
	 ...
	 */
	public BayesPm(Dag graph, int lowerBound, int upperBound) {
		if (graph == null) {
		...
		this.nodesToVariables = new LinkedHashMap<Node, DiscreteVariable>();
	 ...
	 * "valuen".
	 */
	public BayesPm(Dag graph, BayesPm oldBayesPm, int lowerBound, int upperBound) {
		...
		if (graph.getNumNodes() == 0) {
			throw new IllegalArgumentException("The graph must have at least "
					+ "one node in it.");
		}
	...
	public BayesPm(BayesPm bayesPm) {
		...
		for (Node node : bayesPm.nodesToVariables.keySet()) {
			...
			//newVariable.setName(node.getName());
	...
	public void setCategoriesSFU(Node node, List<String> categories) {
		for (Node oldNode : this.nodesToVariables.keySet()) {
			if (oldNode.getName().equals(node.getName())) {
				mapNodeToVariable(oldNode, categories);
			}
		}
	}
	...
	//elwin added
//	public void deleteCategorie(Node node, String categorie){
//		for (Node oldNode : this.nodesToVariables.keySet()) {
//			if (oldNode.getName().equals(node.getName())) {
//
//				List<String> nodecate = nodesToVariables.get(oldNode).getCategories();
//				nodecate.remove(categorie);
//				mapNodeToVariable(oldNode,nodecate);
//			}
//		}
//
//	}
	...
	private void copyAvailableInformationFromOldBayesPm(BayesPm oldbayesPm,
		...
		for (Node node2 : newGraph.getNodes()) {
			if (oldGraph.containsNode(node2)) {
				DiscreteVariable node = oldbayesPm.nodesToVariables.get(node2);
				node.setNodeType(node2.getNodeType());
				this.nodesToVariables.put(node2, node);
			} else {
	...
	private void mapNodeToVariable(Node node, List<String> categories) {
		if (categories.size() != new LinkedHashSet<String>(categories).size()) {
			throw new IllegalArgumentException("Duplicate variable names.");
		}

		DiscreteVariable variable = new DiscreteVariable(node.getName(),
				categories);
	...
	private static int pickNumVals(int lowerBound, int upperBound) {
		if (lowerBound < 2) {
			throw new IllegalArgumentException("Lower bound must be >= 2: "
					+ lowerBound);
		}

		if (upperBound < lowerBound) {
			throw new IllegalArgumentException("Upper bound must be >= lower "
					+ "bound.");
		}
