 ...
 * @version $Revision: 6118 $ $Date: 2006-01-11 13:12:46 -0500 (Wed, 11 Jan
 *          2006) $
 * @see Session
 * @see SessionListener
 * @see SessionAdapter
...
public class SessionNode implements TetradSerializable {
    ...
    /**
     * Stores a reference to the previous (destroyed) model so that information
...
//        ModificationRegistery.registerModel(this);
     ...
     */
    public boolean addParent(SessionNode parent, Session session) {
        if (this.parents.contains(parent)) {
            return false;
        }
...
//        }
    ...
    public boolean useClonedModel() {
        try {
            ...
            this.model = (SessionModel) new MarshalledObject(model).get();
            this.model.setName(getDisplayName());

            if (this.model instanceof ParamsResettable &&
                    temp instanceof ParamsResettable) {
                Object resettableParams =
                        ((ParamsResettable) temp).getResettableParams();
     ...
     */
    Object[] assignParameters(Class[] parameterTypes, List objects)
    ...
    /**
     * Returns the outerProduct of the entry in the given array.
     */
    ...
    private void createModelUsingArguments(Class modelClass, List models)
        ...
        for (Constructor constructor : constructors) {
            ...
            if (arguments != null) {
                try {
                    this.model =
                            (SessionModel) constructor.newInstance(arguments);
                    this.model.setName(getDisplayName());
                }
                    ...
                    String name =
                            packagePath.substring(begin, packagePath.length());
    ...
    /**
     * Reassesses whether the current model is permitted in light of the
    ...
    private class SessionHandler extends SessionAdapter {
		...
		public void executionStarted(SessionEvent event) {
            ...
            // Restart the current param object if necessary.
