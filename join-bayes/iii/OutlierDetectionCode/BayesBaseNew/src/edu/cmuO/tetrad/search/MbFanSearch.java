 ...
 * Note that this code has been converted to be consistent with the CPC
 * algorithm.
 *
 * @author Joseph Ramsey
 */
public final class MbFanSearch implements MarkovBlanketSearch, GraphSearch {
    ...
    private Map<Node, List<Node>> removedEdges = new HashMap<Node, List<Node>>();
     ...
     */
    public MbFanSearch(IndependenceTest test, int depth) {
        if (test == null) {
    ...
    public Graph search(String targetName) {
        ...
        for (Node v : graph.getAdjacentNodes(getTarget())) {
            ...
            // Look at adjacents w of v. If w is not in A, and there is no
            ...
            for (Node w : graph.getAdjacentNodes(v)) {
                ...
                while ((choice = cg.next()) != null) {
                    List<Node> s = SearchGraphUtils.asList(choice, adjT);
                    if (!s.contains(v)) continue;

                    if (test.isIndependent(getTarget(), w, s)) {
                        addRemovedEdge(v, w);
        ...
        TetradLogger.getInstance().info("BEGINNING step 3 (prune PCPC).");
        ...
        TetradLogger.getInstance().log("graph","After step 3 (prune PCPC)" + graph);
        ...
        TetradLogger.getInstance().info("BEGINNING step 6 (Remove edges among P and P of C).");
...
//        TetradLogger.getInstance().info("target = " + getTarget());
    ...
    /**
     * Returns the set of triples identified as ambiguous by the CPC algorithm
    ...
    /**
     * Returns the set of triples identified as colliders by the CPC algorithm
     * during the most recent search.
     */
    ...
    /**
     * Returns the set of triples identified as noncolliders by the CPC
     * algorithm during the most recent search.
     */
    ...
    /**
     * Returns the number of independence tests performed during the most
     * recent search.
     */
    @Override
    ...
    /**
     * Sets the true MB; should be done before running the search to get
    ...
    private void addAllowableAssociates(Node v, Graph graph) {
        ...
        for (Node w : variables) {
            if (getA().contains(w) && !isRemovedEdge(v, w)) {
                continue;
            }
...
//        System.out.println("***************NUMASSOC = " + numAssociated);
    ...
    /**
     * Tries node remove the edge node---from using adjacent nodes of node
    ...
    private void prune(Node node, Graph graph, int depth) {
        TetradLogger.getInstance().details("Trying to remove edges adjacent to node " + node +
                ", depth = " + depth + ".");
    ...
    private boolean edgeForbidden(Node x1, Node x2) {
        return getKnowledge().edgeForbidden(x1.toString(), x2.toString()) &&
                getKnowledge().edgeForbidden(x2.toString(), x1.toString());
    }

    private boolean edgeRequired(Node x1, Node x2) {
        return getKnowledge().edgeRequired(x1.toString(), x2.toString()) ||
                getKnowledge().edgeRequired(x2.toString(), x1.toString());
    }

    /**
     * Removes from adjx any that cannot be parents of x given the background
     * knowledge.
     */
    ...
    /**
     * Returns true just in case z is a possible parent of x, in the sense that
     * edges are not forbidden from z to x, and edges are not required from
     * either x to z, according to background knowledge.
     */
    ...
    private void addRemovedEdge(Node v, Node w) {
        if (removedEdges.get(v) == null) {
            removedEdges.put(v, new LinkedList<Node>());
        }

        List<Node> removed = removedEdges.get(v);
        if (!removed.contains(w)) removed.add(w);
    }

    private boolean isRemovedEdge(Node v, Node w) {
        return getRemovedEdges(w).contains(v) || getRemovedEdges(v).contains(w);
    }

    private List<Node> getRemovedEdges(Node v) {
        List<Node> removed = removedEdges.get(v);
        return removed == null ? new LinkedList<Node>() : removed;
    }
